...
async function runTransactionMigration() {
  ...
  try {
    ...
    const migrationPath = path.join(__dirname, '..', 'migration_transaction_support.sql');
      ...
      if (statement.trim()) {
        ...
        try {
          ...
          if (error) {
            ...
            const { error: directError } = await supabase.from('pg_stat_activity').select('*').limit(1);
            if (directError) {
              ...
              console.log('   ⚠️  Transaction functions need to be created manually in Supabase dashboard');
          ...
          console.log('   ⚠️  Please create this function manually in Supabase SQL editor:');
    ...
    console.log('⚠️  Please verify transaction functions were created in Supabase dashboard');
