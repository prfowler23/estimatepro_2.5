-- ============================================
-- PERFORMANCE OPTIMIZATION INDEXES - SIMPLE VERSION
-- ============================================
-- Only basic column indexes, no functions or casting
...
-- Company name index if column exists
CREATE INDEX IF NOT EXISTS idx_estimates_company_name 
ON estimates(company_name) WHERE company_name IS NOT NULL;
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'estimate_services') THEN
        ...
        CREATE INDEX IF NOT EXISTS idx_estimate_services_created_at 
        ON estimate_services(created_at) WHERE created_at IS NOT NULL;
    END IF;
