...
describe('Service Validation Rules', () => {
  ...
  test('WC cannot exist without PW or PWS', () => {
    const result = SERVICE_RULES.validateServiceSelection(['WC']);
    expect(result.errors).toContain('Window Cleaning requires either Pressure Washing or Pressure Wash & Seal');
  });
  
  test('PWS must be before GR and FR', () => {
    const services = ['GR', 'PWS', 'FR'];
  ...
  test('PW and PWS together generates warning', () => {
    ...
    expect(result.warnings).toContain('Both PW and PWS selected - typically only one is needed');
  });

  test('GR without FR suggests adding FR', () => {
    const result = SERVICE_RULES.validateServiceSelection(['GR']);
    expect(result.warnings).toContain('Consider adding FR - commonly done with GR');
  });

  test('FR without GR suggests adding GR', () => {
    const result = SERVICE_RULES.validateServiceSelection(['FR']);
    expect(result.warnings).toContain('Consider adding GR - commonly done with FR');
  });
  ...
  test('Valid service combination returns no errors', () => {
    const result = SERVICE_RULES.validateServiceSelection(['PWS', 'GR', 'FR', 'WC']);
    expect(result.valid).toBe(true);
    expect(result.errors).toHaveLength(0);
  });
  ...
  test('Independent services work alone', () => {
    SERVICE_RULES.independentServices.forEach(service => {
      const result = SERVICE_RULES.validateServiceSelection([service]);
      expect(result.valid).toBe(true);
      expect(result.errors).toHaveLength(0);
    });
  });
  ...
  test('Service order priority is respected', () => {
    const services = ['WC', 'PW', 'GR'];
    const result = SERVICE_RULES.validateServiceSelection(services);
    
    // Should suggest proper ordering
    expect(result.warnings.some(w => w.includes('should be scheduled'))).toBe(true);
  });
  ...
  test('Complex service combination validation', () => {
    const result = SERVICE_RULES.validateServiceSelection(['PWS', 'GR', 'FR', 'WC', 'HD', 'FC']);
    expect(result.valid).toBe(true);
  ...
  test('Sequence rules validation with proper order', () => {
    const result = SERVICE_RULES.validateServiceSelection(['PWS', 'WC', 'GR', 'FR']);
    expect(result.valid).toBe(true);
    // Should have minimal warnings since order is correct
  });

  test('Sequence rules validation with wrong order', () => {
    const result = SERVICE_RULES.validateServiceSelection(['GR', 'FR', 'PWS', 'WC']);
    expect(result.valid).toBe(true); // Still valid but with warnings
    expect(result.warnings.some(w => w.includes('should be scheduled after'))).toBe(true);
  });

  test('Service validation is case insensitive for existing services', () => {
    // Test that our validation works with the exact service codes
    const result = SERVICE_RULES.validateServiceSelection(['pw']);
    // Should not auto-add since 'pw' is not recognized as 'PW'
    expect(result.autoAddedServices).toHaveLength(0);
  });

  test('All service priorities are defined', () => {
    const allServices = ['PW', 'PWS', 'WC', 'GR', 'FR', 'HD', 'SW', 'PD', 'GRC', 'FC'];
