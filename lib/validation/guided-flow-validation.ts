...
export const FilesPhotosValidationSchema = z.object({
  ...
  summary: z.object({
    totalPhotos: z
      .number()
      .min(1, "At least one photo is required for analysis"),
...
export const AreaOfWorkValidationSchema = z.object({
  ...
  scale: z.object({
    ...
    isSet: z
      .boolean()
      .refine((val) => val === true, "Scale must be properly set"),
  }),
...
export const SummaryValidationSchema = z.object({
  customerApproval: z
    .boolean()
    .refine((val) => val === true, "Customer approval is required"),
...
export interface ValidationResult {
  ...
  warnings: string[];
  suggestions?: string[];
  businessRules?: ValidationReport;
  qualityScore?: number;
...
export class GuidedFlowValidator {
  static validateStep(
    ...
    try {
      ...
      if (error instanceof z.ZodError) {
        schemaErrors = error.errors.map(
          (err) => `${err.path.join(".")}: ${err.message}`,
        );
      } else {
        schemaErrors = ["Unknown validation error"];
      }
    }

    // Business rule validation (if full flow data is provided)
    let businessRules: ValidationReport | undefined;
    let suggestions: string[] = [];
    let qualityScore = schemaValid ? 85 : 50; // Base score
    ...
    if (flowData) {
      try {
        ...
        businessRules = BusinessRuleEngine.executeRules(ruleContext);

        // Extract suggestions from business rules
        suggestions = businessRules.suggestions.map((s) => s.message);

        // Use business rule quality score if available
        if (businessRules.totalScore > 0) {
          qualityScore = schemaValid
            ? (businessRules.totalScore + 85) / 2 // Average with schema score
            : businessRules.totalScore * 0.6; // Reduced for schema errors
        }

        autoFixAvailable = businessRules.autoFixAvailable;
      } catch (error) {
        console.warn("Business rule validation failed:", error);
      }
    }

    // Combine traditional warnings with business rule insights
    const traditionalWarnings = this.getStepWarnings(stepNumber, data);
    const businessWarnings =
      businessRules?.warnings.map((w) => w.message) || [];
    const allWarnings = [...traditionalWarnings, ...businessWarnings];

    // Combine errors
    const businessErrors = businessRules?.errors.map((e) => e.message) || [];
    const allErrors = [...schemaErrors, ...businessErrors];

    return {
      isValid: schemaValid && businessErrors.length === 0,
      errors: allErrors,
  ...
  static validateAllSteps(flowData: any, userProfile?: any): ValidationResult {
    ...
    for (let i = 1; i <= 9; i++) {
      ...
      if (stepData) {
        const result = this.validateStep(i, stepData, flowData, userProfile);
        allErrors.push(...result.errors.map((err) => `Step ${i}: ${err}`));
        allWarnings.push(
          ...result.warnings.map((warn) => `Step ${i}: ${warn}`),
        );
        if (result.suggestions) {
          allSuggestions.push(
            ...result.suggestions.map((sug) => `Step ${i}: ${sug}`),
          );
        }
    ...
    return {
      ...
      qualityScore: stepCount > 0 ? totalQualityScore / stepCount : 0,
      autoFixAvailable,
    };
  }

  // Apply auto-fixes using business rule engine
  static applyAutoFixes(
    ...
    try {
      ...
      return BusinessRuleEngine.applyAutoFixes(ruleContext, ruleCodes);
    } catch (error) {
      console.error("Failed to apply auto-fixes:", error);
      return flowData; // Return original data if auto-fix fails
    }
  }

  // Get comprehensive validation report for a step
  static getStepValidationReport(
    stepNumber: number,
    flowData: any,
    userProfile?: any,
  ): {
    ...
    const stepData = this.getStepData(stepNumber, flowData);
    const basicValidation = stepData
      ? this.validateStep(stepNumber, stepData, flowData, userProfile)
      : { isValid: false, errors: ["No data for this step"], warnings: [] };
    ...
    const businessRules = BusinessRuleEngine.executeRules(ruleContext);

    const recommendations = [
      ...(basicValidation.suggestions || []),
      ...businessRules.suggestions.map((s) => s.message),
    ];

    const complianceIssues = businessRules.complianceIssues.map(
      (c) => `${c.message} (${c.compliance?.standard})`,
    );
  ...
  static getStepWarnings(stepNumber: number, data: any): string[] {
    ...
    switch (stepNumber) {
      case 1: // Initial Contact
        if (
          !data.extractedData?.customer?.email &&
          !data.extractedData?.customer?.phone
        ) {
          warnings.push(
            "No contact information extracted - consider manual entry",
          );
        }
        break;
      ...
      case 2: // Scope Details
        if (data.selectedServices.length > 5) {
          warnings.push(
            "Many services selected - ensure proper scheduling coordination",
          );
        }
        break;
      ...
      case 3: // Files/Photos
        if (data.summary.analyzedPhotos < data.summary.totalPhotos) {
          warnings.push(
            "Not all photos have been analyzed - some estimates may be incomplete",
          );
        }
        break;
      ...
      case 4: // Area of Work
        if (data.workAreas.some((area: any) => area.area > 50000)) {
          warnings.push(
            "Large work areas detected - consider dividing into smaller sections",
          );
        }
        break;
      ...
      case 5: // Takeoff
        if (data.measurements.length < 3) {
          warnings.push(
            "Limited measurements - consider adding more detail for accuracy",
          );
        }
        break;
      ...
      case 6: // Duration
        if (data.timeline.totalDays > 30) {
          warnings.push(
            "Long project duration - consider milestone scheduling",
          );
        }
        break;
      ...
      case 7: // Expenses
        if (data.totalCosts.grand > 100000) {
          warnings.push(
            "High-value project - ensure thorough review before submission",
          );
        }
        break;
