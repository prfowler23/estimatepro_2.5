...
// Import the new dynamic service
...
export class MarketAnalysisService {
  ...
  private async getCompetitorData(
    ...
    // Try to use dynamic competitive intelligence first
    try {
      const dynamicData = EquipmentMaterialsService.getCompetitorPricing(
        location,
        services[0],
      );
      if (dynamicData) {
        return this.convertDynamicCompetitors(dynamicData, services);
      }
    } catch (error) {
      console.warn(
        "Failed to load dynamic competitor data, using static fallback:",
        error,
      );
    }
  ...
  private competitorOffersServices(
  ...
  private getRegionalMultiplier(location: string): number {
    // Try to use dynamic cost of living data first
    try {
      const costMultiplier =
        EquipmentMaterialsService.getCostOfLivingMultiplier(location);
      if (costMultiplier && costMultiplier > 0) {
        return costMultiplier;
      }
    } catch (error) {
      console.warn(
        "Failed to get dynamic cost multiplier, using static data:",
        error,
      );
    }

    // Fallback to static regional pricing variations
    ...
    const key = location.toLowerCase().replace("-", "").replace(" ", "");
  ...
  private calculatePriceDistribution(
    ...
    const basePrices: Record<string, number> = {
      ...
      PWS: 1.3, // Pressure washing with sealing per sq ft
      WC: 3.0, // Window cleaning per sq ft
      GR: 20.0, // Graffiti removal per sq ft
      FR: 25.0, // Full restoration per sq ft
      HBW: 2.5, // High building work per sq ft
      SWR: 45.0, // Stone/brick restoration per sq ft
      RESTO: 60.0, // Full building restoration per sq ft
    };

    const totalBasePrice = services.reduce(
      (sum, service) => sum + (basePrices[service] || 0) * projectSize,
      0,
    );

    // Apply competitor multipliers
    const competitorPrices = competitors.map(
      (c) => totalBasePrice * c.avgPricing.multiplier,
    );
    ...
    return {
      ...
      p25:
        competitorPrices[Math.floor(competitorPrices.length * 0.25)] ||
        competitorPrices[0],
      median:
        competitorPrices[Math.floor(competitorPrices.length * 0.5)] ||
        competitorPrices[0],
      p75:
        competitorPrices[Math.floor(competitorPrices.length * 0.75)] ||
        competitorPrices[competitorPrices.length - 1],
      max: Math.min(
        competitorPrices[competitorPrices.length - 1] * 1.1,
        totalBasePrice * 2.0,
      ),
    };
  }
  ...
  private async assessDemandLevel(
    ...
    const demandScore = Object.entries(factors).reduce(
      (score, [key, value]) =>
  ...
  private analyzeSeasonality(date: Date, services: string[]): Seasonality {
    ...
    // Try to use dynamic seasonal data first
    try {
      const seasonalMultiplier =
        EquipmentMaterialsService.getSeasonalMultiplier(
          "default", // Use default region or pass actual location
          date.toLocaleDateString("en-US", { month: "long" }).toLowerCase(),
        );

      if (seasonalMultiplier && seasonalMultiplier > 0) {
        // Use dynamic seasonal data
        const nextMonth = new Date(date.getFullYear(), date.getMonth() + 1, 1);
        const nextMultiplier = EquipmentMaterialsService.getSeasonalMultiplier(
          "default",
          nextMonth
        ...
        const trend =
          Math.abs(nextMultiplier - seasonalMultiplier) < 0.05
            ? "stable"
            : nextMultiplier > seasonalMultiplier
              ? "increasing"
              : "decreasing";
      ...
      console.warn(
        "Failed to get dynamic seasonal data, using static fallback:",
        error,
      );
    }
    ...
    const avgMultiplier =
      multipliers.reduce((a, b) => a + b, 0) / (multipliers.length || 1);

    // Determine trend by comparing with next month
    const nextMonth = (month + 1) % 12;
    const nextMultipliers = services.map(
      (s) => seasonalFactors[s]?.[nextMonth] || 1.0,
    );
    const nextAvg =
      nextMultipliers.reduce((a, b) => a + b, 0) /
      (nextMultipliers.length || 1);

    const trend =
      Math.abs(nextAvg - avgMultiplier) < 0.05
        ? "stable"
        : nextAvg > avgMultiplier
          ? "increasing"
          : "decreasing";
  ...
  private calculateConfidence(
    ...
    const avgCompetitorConfidence =
      competitors.reduce((sum, c) => sum + c.avgPricing.confidence, 0) /
  ...
  public getMarketInsights(analysis: MarketAnalysis): string[] {
    ...
    const premiumCompetitors = analysis.competitors.filter(
      (c) => c.tier === "premium",
    ).length;
    const budgetCompetitors = analysis.competitors.filter(
      (c) => c.tier === "budget",
    ).length;

    if (premiumCompetitors > budgetCompetitors) {
      insights.push(
        "Premium-focused market - quality differentiation important",
      );
  ...
  /**
   * Convert dynamic competitor data to our format
   */
  private convertDynamicCompetitors(
    dynamicData: any,
    services: string[],
  ): CompetitorProfile[] {
    const competitors: CompetitorProfile[] = [];

    // Convert market leader
    if (dynamicData.marketLeader) {
      competitors.push({
        name: dynamicData.marketLeader.name,
        tier: this.mapTierFromDynamic(dynamicData.marketLeader.tier),
        marketShare: dynamicData.marketLeader.marketShare,
        avgPricing: {
          multiplier: dynamicData.marketLeader.averagePricingMultiplier,
          confidence: 0.9,
        },
        strengths: dynamicData.marketLeader.strengths || [],
        weaknesses: dynamicData.marketLeader.weaknesses || [],
        typicalDiscount: this.getDiscountByTier(dynamicData.marketLeader.tier),
        responseTime: 12,
        serviceQuality: this.getQualityByTier(dynamicData.marketLeader.tier),
        insuranceCoverage: this.getInsuranceByTier(
          dynamicData.marketLeader.tier,
        ),
      });
    }

    // Convert budget option
    if (
      dynamicData.budgetOption &&
      dynamicData.budgetOption.name !== dynamicData.marketLeader.name
    ) {
      competitors.push({
        name: dynamicData.budgetOption.name,
        tier: this.mapTierFromDynamic(dynamicData.budgetOption.tier),
        marketShare: dynamicData.budgetOption.marketShare || 15,
        avgPricing: {
          multiplier: dynamicData.budgetOption.averagePricingMultiplier,
          confidence: 0.8,
        },
        strengths: dynamicData.budgetOption.strengths || ["Low pricing"],
        weaknesses: dynamicData.budgetOption.weaknesses || ["Quality concerns"],
        typicalDiscount: this.getDiscountByTier(dynamicData.budgetOption.tier),
        responseTime: 24,
        serviceQuality: this.getQualityByTier(dynamicData.budgetOption.tier),
        insuranceCoverage: this.getInsuranceByTier(
          dynamicData.budgetOption.tier,
        ),
      });
    }

    // Add additional synthetic competitors to fill out the market
    if (competitors.length < 3) {
      const marketData = EquipmentMaterialsService.getMarketData(location);
      if (marketData?.competitors) {
        marketData.competitors.forEach((comp) => {
          if (!competitors.find((c) => c.name === comp.name)) {
            competitors.push({
              name: comp.name,
              tier: this.mapTierFromDynamic(comp.tier),
              marketShare: comp.marketShare,
              avgPricing: {
                multiplier: comp.averagePricingMultiplier,
                confidence: 0.75,
              },
              strengths: comp.strengths || [],
              weaknesses: comp.weaknesses || [],
              typicalDiscount: this.getDiscountByTier(comp.tier),
              responseTime: 18,
              serviceQuality: this.getQualityByTier(comp.tier),
              insuranceCoverage: this.getInsuranceByTier(comp.tier),
            });
          }
        });
      }
  ...
  /**
   * Map dynamic tier to our tier format
   */
  private mapTierFromDynamic(tier: string): CompetitorProfile["tier"] {
    const tierMap: Record<string, CompetitorProfile["tier"]> = {
      budget: "budget",
      standard: "standard",
      premium: "premium",
      luxury: "luxury",
  ...
  /**
   * Get service quality score by tier
   */
  private getQualityByTier(tier: string): number {
    ...
    return qualityMap[tier] || 85;
  }

  /**
   * Get insurance coverage description by tier
   */
  private getInsuranceByTier(tier: string): string {
    const insuranceMap: Record<string, string> = {
      budget: "$1M General",
      standard: "$2M General",
      premium: "$5M General + $2M Professional",
      luxury: "$10M General + $5M Professional",
    };
    return insuranceMap[tier] || "$2M General";
  }
}
