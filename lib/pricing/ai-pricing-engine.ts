...
export class AIPricingEngine {
  async calculateOptimalPrice(
    ...
    const markedUpCost = this.calculateMarkedUpCost(
      factors.baseCost,
      factors.margins,
    );
    ...
    return {
      ...
      alternativeStrategies: strategiesWithProbability.filter(
        (s) => s !== optimalStrategy,
      ),
  ...
  private calculateMarkedUpCost(baseCost: number, margins: any): number {
    ...
    const weightedMargin =
      (margins.equipment * 0.3 +
        margins.materials * 0.2 +
        margins.labor * 0.5) /
      100;
  ...
  private async analyzeMarketPosition(
    ...
    const competitiveness = this.assessCompetitiveness(
      percentile,
  ...
  private assessCompetitiveness(
    ...
    if (percentile < 25)
      competitiveness = 1.2; // Very competitive
    else if (percentile < 50)
      competitiveness = 1.1; // Competitive
    else if (percentile > 75) competitiveness = 0.9; // Premium

    // Adjust for demand
    if (demandLevel > 80)
      competitiveness *= 1.05; // High demand allows higher prices
    else if (demandLevel < 40) competitiveness *= 0.95; // Low demand requires lower prices

    return competitiveness;
  }

  private getMarketRecommendation(
    percentile: number,
    competitiveness: number,
  ): string {
    if (percentile < 25)
      return "Highly competitive pricing - consider value-adds";
  ...
  private analyzeCompetitors(
    ...
    const avgCompetitorPrice =
      relevantCompetitors.length > 0
        ? relevantCompetitors.reduce(
            (sum, comp) => sum + comp.averagePrice,
            0,
    ...
    return {
      ...
      pricePosition,
      strongestCompetitor: relevantCompetitors.sort(
  ...
  private generateCompetitorInsights(
    ...
    if (pricePosition > 1.2) {
      insights.push(
        "Pricing significantly above competitors - ensure strong value proposition",
      );
  ...
  private scoreQualityExpectation(previousVendors: string[]): number {
    ...
    const hasPremium = previousVendors.some((vendor) =>
      premiumVendors.some((premium) =>
        vendor.toLowerCase().includes(premium.toLowerCase()),
      ),
    );
  ...
  private calculateConfidence(profile: CustomerProfile): number {
    ...
    if (profile.priority === "price" && profile.budgetIndicators === "tight")
      confidence += 10;
    if (
      profile.priority === "quality" &&
      profile.budgetIndicators === "premium"
    )
      confidence += 10;
  ...
  private calculateRiskAdjustment(
    ...
    const riskServiceCount = services.filter((s) =>
      highRiskServices.includes(s),
    ).length;
  ...
  private optimizeBundlePricing(
    ...
    const bundleWinRate = this.getBundleWinRate(
      bestBundle.services,
      historicalData,
    );
    const adjustedDiscount =
      bundleWinRate > 0.7 ? bestBundle.discount * 0.8 : bestBundle.discount;
  ...
  private getBundleWinRate(
    ...
    return (
      bundleData.reduce((sum, data) => sum + data.winRate, 0) /
      bundleData.length
    );
  }
  ...
  private generatePricingStrategies(
    ...
    strategies.push({
      ...
      adjustments: [
        {
          reason: "Customer value perception",
          percentage: (customerWTP.score - 0.5) * 40,
        },
        {
          reason: "Project risk factors",
          percentage: (riskAdjustment - 1) * 100,
        },
      ],
      pros: [
        "Maximizes profit on high-value customers",
        "Reflects true project value",
      ],
      ...
      adjustments: [
        {
          ...
          percentage: (marketPosition.competitiveness - 1) * 100,
        },
      ],
    ...
    if (marketPosition.percentile > 60) {
      strategies.push({
        ...
        adjustments: [{ reason: "Penetration discount", percentage: -15 }],
    ...
    if (bundleOpportunity.applicable) {
      strategies.push({
        ...
        adjustments: [
          {
            reason: `${bundleOpportunity.name} bundle`,
            percentage: -bundleOpportunity.discount,
          },
        ],
  ...
  private calculateWinProbability(
    ...
    // Adjust for market conditions
    const marketAdjustment =
      marketPosition.percentile < 40
        ? 1.2
        : marketPosition.percentile > 70
          ? 0.8
          : 1.0;

    // Adjust for customer profile
    const customerAdjustment =
      customerWTP.score > 0.7 ? 1.1 : customerWTP.score < 0.3 ? 0.9 : 1.0;

    // Calculate final probability
    const probability =
      historicalWinRate * marketAdjustment * customerAdjustment;

    // Ensure within bounds
    return Math.max(0.1, Math.min(0.95, probability));
  }

  private getHistoricalWinRate(
    price: number,
    historicalData: WinRateData[],
  ): number {
    ...
    const sortedData = [...historicalData].sort(
      (a, b) => Math.abs(a.pricePoint - price) - Math.abs(b.pricePoint - price),
    );
    ...
    switch (priority) {
      case "price":
        ...
        return strategies.reduce((best, current) =>
          (current.winProbability || 0) > (best.winProbability || 0)
            ? current
            : best,
        );
  ...
  private generateInsights(
    ...
    if (strategy.name === "Value Optimized" && customerWTP.score > 0.7) {
      insights.push(
        "Customer shows high willingness to pay - value strategy well-suited",
      );
    }
    ...
    if (strategy.winProbability && strategy.winProbability > 0.8) {
      insights.push(
        "High win probability - pricing is well-calibrated for this customer",
      );
    }
