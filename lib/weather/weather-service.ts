...
export class WeatherService {
  ...
  // Enhanced method that tries real APIs first, falls back to mock data
  async analyzeWeatherForLocation(
    ...
    try {
      // Try to get real weather data using enhanced service
      if (this.apiKey) {
  ...
  // New method to get real weather data from APIs
  private async getRealWeatherData(
    ...
    try {
      ...
      const processedData = this.processRealWeatherData(
        currentData,
        forecastData,
        services,
        proposedStartDate,
  ...
  // Process real weather API data into our format
  private processRealWeatherData(
    currentData: any,
    forecastData: any,
    services: string[],
    proposedStartDate: Date,
    ...
    const historical = {
      fiveYearAverages: this.generateHistoricalData(locationInfo.name),
      extremeEvents: [] as WeatherEvent[],
  ...
  // Calculate workability from real weather data
  private calculateRealWorkability(dayData: any): number {
    let score = 1.0;
  ...
  private calculateWeeklyRiskLevel(
  ...
  // Calculate service impacts from real forecast data
  private calculateServiceImpactsFromReal(
    services: string[],
  ...
  // Calculate impact factors from real data
  private calculateImpactFactorsFromReal(
    service: string,
    forecast: DailyForecast[],
  ): {
  ...
  // Calculate delay risk from real data
  private calculateDelayRiskFromReal(
    service: string,
    forecast: DailyForecast[],
  ): number {
    const sensitivity = this.getWeatherSensitivity(service);
  ...
  // Generate service recommendations from real data
  private getServiceSpecificRecommendationsFromReal(
    service: string,
    forecast: DailyForecast[],
  ): string[] {
  ...
  // Generate recommendations from real weather data
  private generateRecommendationsFromReal(
    services: string[],
    forecast: DailyForecast[],
    alerts: any[],
  ): string[] {
  ...
  // Find longest workable streak in forecast
  private findLongestWorkableStreak(forecast: DailyForecast[]): number {
    let longestStreak = 0;
    let currentStreak = 0;
  ...
  // Find optimal work periods in real forecast
  private findOptimalPeriodsInReal(
    forecast: DailyForecast[],
