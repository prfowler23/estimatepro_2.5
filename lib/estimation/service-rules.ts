...
export const SERVICE_RULES = {
  ...
  serviceOrder: {
    ...
    SW: { priority: 2 },
    BF: { priority: 2, mustBeBefore: ["GR", "FR"] }, // Biofilm removal before restoration
  ...
  validateServiceSelection(services: ServiceType[]): {
    valid: boolean;
    errors: string[];
    warnings: string[];
    autoAddedServices: ServiceType[];
  } {
    const errors: string[] = [];
    const warnings: string[] = [];
    const autoAddedServices: ServiceType[] = [];

    // Check mandatory pairs
    Object.entries(this.mandatoryPairs).forEach(
      ([service, requiredServices]) => {
        if (services.includes(service as ServiceType)) {
          requiredServices.forEach((requiredService) => {
            if (!services.includes(requiredService as ServiceType)) {
              autoAddedServices.push(requiredService as ServiceType);
            }
          });
        }
      },
        ...
        rule.after.forEach((afterService) => {
          if (services.includes(afterService)) {
            ...
            if (afterIndex < beforeIndex) {
              warnings.push(
                `${afterService} should be scheduled after ${rule.before} for optimal results`,
              );
            }
          }
        });
    ...
    this.commonPairs.forEach((pair) => {
      if (services.includes(pair[0]) && !services.includes(pair[1])) {
        warnings.push(
          `Consider adding ${pair[1]} - commonly done with ${pair[0]}`,
        );
      } else if (services.includes(pair[1]) && !services.includes(pair[0])) {
        warnings.push(
          `Consider adding ${pair[0]} - commonly done with ${pair[1]}`,
        );
      }
    });

    // Validate service order compatibility
    const selectedWithOrder = services.filter(
      (service) => this.serviceOrder[service as keyof typeof this.serviceOrder],
    );
    selectedWithOrder.forEach((service) => {
      const serviceRules =
        this.serviceOrder[service as keyof typeof this.serviceOrder];
      ...
      if ("mustBeBefore" in serviceRules && serviceRules.mustBeBefore) {
        serviceRules.mustBeBefore.forEach((afterService) => {
          if (services.includes(afterService)) {
            ...
            const afterServicePriority =
              this.serviceOrder[afterService as keyof typeof this.serviceOrder]
                ?.priority || 99;
            if (servicePriority >= afterServicePriority) {
              warnings.push(
                `${service} should be scheduled before ${afterService}`,
              );
            }
          }
        });
      ...
      if ("mustBeAfter" in serviceRules && serviceRules.mustBeAfter) {
        serviceRules.mustBeAfter.forEach((beforeService) => {
          if (services.includes(beforeService)) {
            const servicePriority = serviceRules.priority;
            const beforeServicePriority =
              this.serviceOrder[beforeService as keyof typeof this.serviceOrder]
                ?.priority || 1;
            if (servicePriority <= beforeServicePriority) {
              warnings.push(
                `${service} should be scheduled after ${beforeService}`,
              );
            }
          }
        });
