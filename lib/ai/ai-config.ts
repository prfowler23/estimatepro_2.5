...
const DEFAULT_SERVICE_PRICING: ServicePricingConfig = {
  ...
  GRC: {
    ...
    materialMultiplier: parseFloat(
      process.env.GRC_MATERIAL_MULTIPLIER || "1.6",
    ),
    complexityFactors: {
  ...
  PWS: {
    ...
    materialMultiplier: parseFloat(
      process.env.PWS_MATERIAL_MULTIPLIER || "1.4",
    ),
    complexityFactors: {
...
export class AIConfigManager {
  ...
  private loadConfiguration(): void {
    try {
      ...
      // Create configuration with fallback for browser environment
      const configToValidate = {
        ...DEFAULT_AI_CONFIG,
        // In browser, use a placeholder key to prevent validation errors
        openaiApiKey: isBrowser
          ? "browser-placeholder-key"
          : DEFAULT_AI_CONFIG.openaiApiKey,
      };

      // Validate and load AI config
      this.config = aiConfigSchema.parse(configToValidate);
      ...
      if (!isBrowser && !DEFAULT_AI_CONFIG.openaiApiKey) {
        console.warn(
          "OPENAI_API_KEY environment variable not set - AI features will be disabled",
        );
      ...
      // Don't throw error in browser environment, just log it
  ...
  public isAIAvailable(): boolean {
    const isBrowser = typeof window !== "undefined";
    if (isBrowser) {
      // In browser, assume AI is available if we reach this point
      return true;
    }
    // On server, check if we have a real API key
    return (
      !!DEFAULT_AI_CONFIG.openaiApiKey &&
      DEFAULT_AI_CONFIG.openaiApiKey.length > 10
    );
  }
  ...
  public getServicePricing(
    serviceCode?: string,
  ): ServicePricingConfig | ServicePricingConfig[string] {
    if (serviceCode) {
  ...
  public updateServicePricing(
    serviceCode: string,
    pricing: Partial<ServicePricingConfig[string]>,
  ): void {
  ...
  public updateBuildingMultiplier(
    buildingType: string,
    multiplier: number,
  ): void {
    if (multiplier > 0 && multiplier <= 5) {
      ...
      console.info(
        `Updated building multiplier for ${buildingType}: ${multiplier}`,
      );
    }
  }
  ...
  public importConfiguration(config: {
    ...
    if (config.servicePricing) {
      Object.entries(config.servicePricing).forEach(
        ([serviceCode, pricing]) => {
    ...
    if (config.buildingMultipliers) {
      Object.entries(config.buildingMultipliers).forEach(
        ([buildingType, multiplier]) => {
          this.updateBuildingMultiplier(buildingType, multiplier || 1.0);
        },
      );
    }
...
let _aiConfigInstance: AIConfigManager | null = null;

export function getAIConfig(): AIConfigManager {
  if (!_aiConfigInstance) {
    _aiConfigInstance = AIConfigManager.getInstance();
  }
  return _aiConfigInstance;
}
