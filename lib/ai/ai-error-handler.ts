...
export async function withRetry<T>(
  ...
  for (let attempt = 1; attempt <= retryConfig.maxAttempts; attempt++) {
    try {
      ...
      console.warn(`AI operation failed (attempt ${attempt}/${retryConfig.maxAttempts}), retrying in ${delay}ms:`, error.message);
...
export class RateLimiter {
  ...
  async checkRateLimit(key: string): Promise<void> {
    ...
    const requestsLastMinute = validRequests.filter(t => t > minuteAgo).length;
  ...
  getRateLimitStatus(key: string): {
    ...
    return {
      remainingMinute: Math.max(0, this.config.requestsPerMinute - requestsLastMinute),
      remainingHour: Math.max(0, this.config.requestsPerHour - requestsLastHour),
...
export function checkContentSafety(content: string): void {
  ...
  for (const pattern of unsafe_patterns) {
    if (pattern.test(content)) {
      throw new ContentFilterError('Content contains potentially unsafe patterns');
    }
  }
...
export class AILogger {
  ...
  log(entry: Omit<AILogEntry, 'timestamp'>): void {
    ...
    if (entry.success) {
      console.info(`[AI] ${entry.operation} completed in ${entry.duration}ms`, entry.metadata);
    } else {
      console.error(`[AI] ${entry.operation} failed: ${entry.error}`, entry.metadata);
    }
  ...
  getLogs(filter?: Partial<AILogEntry>): AILogEntry[] {
    ...
    return this.logs.filter(log => {
      return Object.entries(filter).every(([key, value]) => 
        log[key as keyof AILogEntry] === value
      );
    });
  }
