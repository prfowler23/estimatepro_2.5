...
export const serviceCodeSchema = z.enum(['PW', 'PWS', 'WC', 'GR', 'FR', 'HD', 'SW', 'PD', 'GRC', 'FC']);
...
export const buildingTypeSchema = z.enum([
  'office', 'retail', 'industrial', 'residential', 'hospital', 
  'school', 'warehouse', 'hotel', 'restaurant', 'mixed-use'
]);
...
export const requirementsSchema = z.object({
  services: z.array(serviceCodeSchema).min(1, 'At least one service must be specified'),
...
export const contentExtractionSchema = z.object({
  ...
  type: z.enum(['email', 'meeting', 'phone', 'walkin', 'pdf', 'rfp', 'contract', 'plans']),
...
// Auto-quote request schema
export const autoQuoteRequestSchema = z.object({
  extractedData: extractedDataSchema,
  buildingPhotos: z.array(z.string().url()).optional(),
  competitorQuotes: z.array(z.string().max(10000)).optional(),
  customPricing: z.record(z.object({
...
export const photoAnalysisSchema = z.object({
  imageUrl: z.string().url('Invalid image URL'),
  analysisType: z.enum(['material_quantities', 'item_count', '3d_reconstruction', 'before_after']),
  userId: z.string().optional()
});

// 3D reconstruction schema
export const threeDReconstructionSchema = z.object({
  imageUrls: z.array(z.string().url()).min(2, 'At least 2 images required for 3D reconstruction').max(10),
  userId: z.string().optional()
});
...
export const emailGenerationSchema = z.object({
  extractedData: extractedDataSchema,
  emailType: z.enum(['initial', 'reminder', 'value_add', 'objection_handling', 'final']),
...
export const competitiveIntelligenceSchema = z.object({
  competitorContent: z.string().min(1, 'Competitor content cannot be empty').max(20000),
  userId: z.string().optional()
});
