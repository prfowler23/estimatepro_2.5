...
export function getFileType(file: File): 'photo' | 'video' | 'area_map' | 'measurement_screenshot' | 'plan' {
  if (file.type.startsWith('image/')) return 'photo';
  if (file.type.startsWith('video/')) return 'video';
  if (file.name.toLowerCase().includes('map')) return 'area_map';
  if (file.name.toLowerCase().includes('measurement')) return 'measurement_screenshot';
  return 'plan';
}

export function calculateTotalWindowArea(windows: Array<{width: number, height: number}>): number {
...
export function aggregateAnalysisResults(analyses: PhotoAnalysisResult[]): {
  ...
  analyses.forEach(analysis => {
    ...
    if (analysis.materials?.breakdown) {
      Object.entries(analysis.materials.breakdown).forEach(([material, percentage]) => {
    ...
    if (analysis.damage) {
      ...
      if (analysis.damage.oxidation) allDamage.push(...analysis.damage.oxidation);
  ...
  const avgSeverity = severityLevels.reduce((sum, level) => {
    return sum + (severityWeights[level as keyof typeof severityWeights] || 1);
...
export function formatAnalysisForDisplay(analysis: PhotoAnalysisResult): {
  ...
  if (analysis.windows) {
    ...
    highlights.push(`Window area: ${analysis.windows.totalArea.toLocaleString()} sq ft`);
  ...
  if (analysis.materials) {
    const dominantMaterial = Object.entries(analysis.materials.breakdown)
      .sort(([,a], [,b]) => b - a)[0];
    ...
    if (analysis.materials.cleaningDifficulty > 7) {
      warnings.push(`High cleaning difficulty (${analysis.materials.cleaningDifficulty}/10)`);
    }

    if (analysis.materials.specialCoatings?.length) {
      highlights.push(`Special coatings: ${analysis.materials.specialCoatings.join(', ')}`);
    }
  }

  if (analysis.measurements) {
    parts.push(`${analysis.measurements.stories} stories`);
    highlights.push(`Building: ${analysis.measurements.buildingHeight}' H × ${analysis.measurements.facadeWidth}' W`);
    
    if (analysis.measurements.confidence < 0.7) {
      warnings.push('Measurement confidence is low - manual verification recommended');
    }
  }
...
export function getRiskLevelBadgeClass(riskLevel: 'low' | 'medium' | 'high'): string {
  switch (riskLevel) {
...
export function validateAnalysisData(analysis: PhotoAnalysisResult): {
  ...
  if (analysis.materials) {
    const totalPercentage = Object.values(analysis.materials.breakdown).reduce((sum, val) => sum + val, 0);
    if (Math.abs(totalPercentage - 100) > 5) {
      warnings.push('Material percentages do not sum to 100%');
    }
    
    if (analysis.materials.cleaningDifficulty < 1 || analysis.materials.cleaningDifficulty > 10) {
  ...
  if (analysis.measurements) {
    ...
    if (analysis.measurements.confidence < 0.3) {
      warnings.push('Very low confidence in measurements - manual verification required');
    }
  }
