...
export function validateBuildingMeasurements(measurements: {
  ...
  if (measurements.glassArea && measurements.facadeArea) {
    ...
    if (ratio > VALIDATION.maxGlassToFacadeRatio) {
      warnings.push(`Glass area (${measurements.glassArea} sq ft) is ${(ratio * 100).toFixed(0)}% of facade area - please verify`)
    }
  }
  
  if (measurements.heightStories && measurements.heightStories > VALIDATION.maxBuildingHeight) {
    warnings.push(`Building height (${measurements.heightStories} stories) is unusually tall - please verify`)
  }
  ...
  if (measurements.heightStories && measurements.heightFeet) {
    ...
    if (avgStoryHeight < 8 || avgStoryHeight > 20) {
      warnings.push(`Average story height (${avgStoryHeight.toFixed(1)} ft) seems unusual`)
    }
  }
...
export function calculateWindowCount(glassArea: number, windowSize: number = 24): number {
...
export function analyzePriceCompetitiveness(
  ...
  const position = (price - marketRange.min) / (marketRange.max - marketRange.min)
...
export function calculateDrops(buildingArea: number, equipmentType: string): number {
...
export function validatePricePerUnit(
  ...
  if (pricePerUnit < expectedRange.min) {
    warnings.push(`Price per ${unitType} ($${pricePerUnit.toFixed(2)}) is below expected range ($${expectedRange.min}-$${expectedRange.max})`)
  }
  
  if (pricePerUnit > expectedRange.max) {
    warnings.push(`Price per ${unitType} ($${pricePerUnit.toFixed(2)}) is above expected range ($${expectedRange.min}-$${expectedRange.max})`)
  }
...
export function validateSetupTimePercentage(
  ...
  if (percentage < VALIDATION.minSetupTimePercent) {
    warnings.push(`Setup time (${(percentage * 100).toFixed(1)}%) is below minimum ${VALIDATION.minSetupTimePercent * 100}%`)
    isValid = false
  }
  
  if (percentage > VALIDATION.maxSetupTimePercent) {
    warnings.push(`Setup time (${(percentage * 100).toFixed(1)}%) exceeds maximum ${VALIDATION.maxSetupTimePercent * 100}%`)
    isValid = false
  }
