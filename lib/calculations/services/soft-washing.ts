...
export class SoftWashingCalculator extends BaseCalculator<SoftWashingInput> {
  calculate(input: SoftWashingInput): CalculationResult {
    ...
    if (includesRoof && roofArea <= 0) {
      throw new Error(
        "Roof area must be greater than 0 when roof cleaning is included",
      );
    }
    ...
    const facadeLaborHours = this.calculateFacadeLaborHours(
      facadeArea,
      surfaceMaterial,
      contaminationLevel,
      crewSize,
    );
    const roofLaborHours = includesRoof
      ? this.calculateRoofLaborHours(roofArea, contaminationLevel, crewSize)
      : 0;
    ...
    this.addBreakdown(
      "Labor Hours",
      includesRoof
        ? `Facade: ${facadeLaborHours.toFixed(1)}h, Roof: ${roofLaborHours.toFixed(1)}h`
        : `Facade: ${facadeLaborHours.toFixed(1)}h`,
    ...
    if (contaminationLevel === "heavy" && finalPrice < 800) {
      this.addWarning(
        "Heavy contamination price seems low - verify measurements and scope",
      );
    }

    if (surfaceMaterial === "wood" && contaminationLevel === "heavy") {
      this.addWarning(
        "Heavy contamination on wood surfaces requires careful chemical selection",
      );
    }

    if (includesRoof && buildingHeightStories > 3) {
      this.addWarning(
        "Roof cleaning on tall buildings requires specialized safety equipment",
      );
    }
    ...
    if (surfaceMaterial === "stucco" && contaminationLevel === "heavy") {
      this.addWarning(
        "Heavy contamination on stucco may require multiple treatments",
      );
    }

    if (includesRoof && roofArea > 8000) {
      this.addWarning(
        "Large roof area - verify safe access and drainage considerations",
      );
    }
    ...
    if (contaminationLevel === "heavy" || surfaceMaterial === "wood") {
      this.addWarning(
        "Chemical selection critical - verify compatibility with surface material",
      );
    }
  ...
  private getFacadeRate(
    ...
    const materialMultiplier =
      materialMultipliers[
        surfaceMaterial as keyof typeof materialMultipliers
      ] || 1.0;
    const contaminationMultiplier =
      contaminationMultipliers[
        contaminationLevel as keyof typeof contaminationMultipliers
      ] || 1.0;
  ...
  private calculateFacadeLaborHours(
    area: number,
    surfaceMaterial: string,
    contaminationLevel: string,
    crewSize: number,
    ...
    const materialFactor =
      materialEfficiency[surfaceMaterial as keyof typeof materialEfficiency] ||
      1.0;
    const contaminationFactor =
      contaminationEfficiency[
        contaminationLevel as keyof typeof contaminationEfficiency
      ] || 1.0;

    const adjustedProductionRate =
      baseProductionRate * materialFactor * contaminationFactor;
    return area / (adjustedProductionRate * crewSize);
  }

  private calculateRoofLaborHours(
    area: number,
    contaminationLevel: string,
    crewSize: number,
  ): number {
  ...
  static calculateChemicalRequirements(
    ...
    const chemicalRates = {
      ...
      bleach: { light: 0.003, moderate: 0.006, heavy: 0.01 },
    ...
    const multiplier =
      surfaceMultipliers[surfaceMaterial as keyof typeof surfaceMultipliers] ||
      1.0;

    const facadeChemicals = {
      detergent:
        facadeArea *
        chemicalRates.detergent[
          contaminationLevel as keyof typeof chemicalRates.detergent
        ] *
        multiplier,
      bleach:
        facadeArea *
        chemicalRates.bleach[
          contaminationLevel as keyof typeof chemicalRates.bleach
        ] *
        multiplier,
      neutralizer:
        facadeArea *
        chemicalRates.neutralizer[
          contaminationLevel as keyof typeof chemicalRates.neutralizer
        ] *
        multiplier,
    };
    ...
    const costs = {
      detergent: 8.5, // per gallon
      bleach: 3.25, // per gallon
      neutralizer: 12.0, // per gallon
    };
    ...
    const totalCost =
      ...
      (facadeChemicals.neutralizer + roofChemicals.neutralizer) *
        costs.neutralizer;
  ...
  static validateEnvironmentalConditions(
    ...
    if (temperature < 50 || temperature > 85) {
      warnings.push(
        "Temperature outside optimal range (50-85°F) for soft washing",
      );
    }
    ...
    if (windSpeed > 12) {
      warnings.push(
        "High wind speed may affect chemical application and safety",
      );
    }

    if (precipitationChance > 25) {
      warnings.push(
        "Precipitation risk - chemicals need time to work before rain",
      );
    }
  ...
  static calculateDwellTime(
    ...
    const tempAdjustment =
      temperature > 80 ? 0.8 : temperature < 60 ? 1.2 : 1.0;

    const dwellTime =
      baseDwellTimes[contaminationLevel as keyof typeof baseDwellTimes] *
  ...
  static calculatePlantProtection(
    ...
    protectionCost =
      totalArea *
      densityMultipliers[landscapingDensity as keyof typeof densityMultipliers];
