...
export class BiofilmRemovalCalculator extends BaseCalculator<BiofilmRemovalInput> {
  calculate(input: BiofilmRemovalInput): CalculationResult {
    ...
    // Sealing surcharge
    if (requiresSealing) {
      const sealingSurcharge = area * 0.20 // $0.20 per sq ft
    ...
    const laborHours = this.calculateLaborHours(area, biofilmSeverity, surfaceType, hasWaterFeatures, crewSize)
    this.addBreakdown(
      'Labor Hours',
    ...
    if (hasWaterFeatures && !requiresSealing) {
      this.addWarning('Water features typically require protective sealing to prevent recurrence')
    }

    if (surfaceType === 'metal' && biofilmSeverity !== 'light') {
      this.addWarning('Metal surfaces with biofilm may show staining - test area recommended')
    }

    if (area > 5000) {
      this.addWarning('Large area treatment - consider environmental impact and runoff management')
    }

    if (biofilmSeverity === 'severe' && finalPrice < 1000) {
      this.addWarning('Severe biofilm price seems low - verify measurements and scope')
    }
    ...
    if (surfaceType === 'stone' && biofilmSeverity === 'severe') {
      this.addWarning('Severe biofilm on stone may require specialized treatment - verify chemical compatibility')
    }

    if (hasWaterFeatures && area > 2000) {
      this.addWarning('Large water feature area - consider phased treatment to minimize disruption')
    }

    // Environmental warnings
    if (biofilmSeverity === 'severe' || area > 3000) {
      this.addWarning('Biofilm treatment chemicals require proper containment and disposal')
    }
  ...
  private getBaseRate(severity: string, surfaceType: string): number {
    ...
    switch (severity) {
      case 'light':
        ...
        break
      case 'moderate':
        rate = (OTHER_RATES.biofilmRemoval.min + OTHER_RATES.biofilmRemoval.max) / 2
        break
      case 'severe':
  ...
  private calculateLaborHours(area: number, severity: string, surfaceType: string, hasWaterFeatures: boolean, crewSize: number): number {
  ...
  static calculateChemicalRequirements(
    ...
    // Calculate costs
    const costs = {
      biocide: biocide * 45.00,      // $45 per gallon
      cleaner: cleaner * 22.50,      // $22.50 per gallon
      sealer: sealer * 38.00,        // $38 per gallon
      neutralizer: neutralizer * 15.00 // $15 per gallon
    }
    
    const totalCost = costs.biocide + costs.cleaner + costs.sealer + costs.neutralizer
    
    // Recommendations
    if (severity === 'severe') {
      recommendations.push('Severe biofilm may require multiple biocide applications')
    }
  ...
  static calculateEnvironmentalImpact(
    ...
    const runoffVolume = area * runoffRates[severity as keyof typeof runoffRates]
    
    // Determine containment requirements
    const containmentRequired = hasWaterFeatures || nearVegetation || area > 1000
    ...
    if (hasWaterFeatures) {
      ...
      recommendations.push('Water features require complete runoff containment')
    }
    ...
    if (severity === 'severe') {
      ...
      recommendations.push('Severe biofilm treatment requires enhanced containment')
    }
    
    if (containmentRequired) {
      recommendations.push('Implement proper containment and collection systems')
    }
  ...
  static calculateTreatmentTimeline(
    ...
    const treatmentHours = area * 0.03 * treatmentMultipliers[severity as keyof typeof treatmentMultipliers]
    ...
    // Sealing phase
    const sealingDays = requiresSealing ? Math.ceil((area * 0.02) / hoursPerDay) : 0
    ...
    // Recommendations
    if (severity === 'severe') {
      recommendations.push('Severe biofilm may require multiple treatment cycles')
    }
    
    if (surfaceType === 'stone') {
      recommendations.push('Allow extended dwell time for porous stone surfaces')
    }
  ...
  static calculatePreventionPlan(
    ...
    if (environment === 'outdoor') {
      if (maintenanceFrequency === 'quarterly') maintenanceFrequency = 'bi-monthly'
    ...
    const cleaningCost = area * 0.15 * frequencyMultipliers[maintenanceFrequency as keyof typeof frequencyMultipliers]
