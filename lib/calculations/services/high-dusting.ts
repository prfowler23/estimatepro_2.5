...
export class HighDustingCalculator extends BaseCalculator<HighDustingInput> {
  calculate(input: HighDustingInput): CalculationResult {
    ...
    if (includesFans) {
      const fanCharge = area * 0.1; // Additional $0.10/sq ft for fans
      basePrice += fanCharge;
      this.addBreakdown(
    ...
    if (includesFans && buildingHeightStories > 10) {
      this.addWarning(
        "High-rise fan cleaning requires specialized equipment and safety measures",
      );
    }

    if (surfaceComplexity === "complex" && projectDays > 5) {
      this.addWarning(
        "Complex high dusting over multiple days - coordinate with building operations",
      );
    }
    ...
    if (includesFans && includesFixtures && area > 10000) {
      this.addWarning(
        "Large area with fans and fixtures - consider phased approach",
      );
    }
    ...
    if (surfaceComplexity === "simple" && (includesFans || includesFixtures)) {
      this.addWarning(
        'Simple surfaces with fans/fixtures may not be truly "simple" - verify complexity',
      );
    }
  ...
  static calculateSpecializedEquipment(
    ...
    if (includesFans) {
      ...
      recommendations.push(
        "Fan cleaning requires specialized brushes and tools",
      );
    }
    ...
    if (includesFixtures && area > 10000) {
      ...
      additionalCost += 75;
      recommendations.push(
        "Large fixture cleaning requires comprehensive tool kit",
      );
    }
    ...
    if (buildingHeight > 15 && (includesFans || includesFixtures)) {
      ...
      additionalCost += 100;
      recommendations.push(
        "High-rise detail work requires enhanced safety equipment",
      );
    }
  ...
  static calculateDustLoadAssessment(
    ...
    if (buildingType === "industrial") {
      ...
      timeMultiplier = 1.5;
      recommendations.push(
        "Industrial environments require heavy-duty cleaning",
      );
    } else if (buildingType === "retail") {
      dustLoad = "moderate";
      timeMultiplier = 1.2;
      recommendations.push(
        "Retail environments have moderate dust accumulation",
      );
    }
    ...
    if (lastCleaning > 12) {
      ...
      timeMultiplier *= 1.3;
      recommendations.push(
        "Long intervals between cleanings increase dust load",
      );
    }
  ...
  static calculateSchedulingConstraints(
    ...
    if (buildingType === "healthcare") {
      ...
      efficiency *= 0.8;
      recommendations.push(
        "Healthcare facilities require specialized cleaning protocols",
      );
    }
    ...
    if (buildingType === "office" && workHours === "business") {
      ...
      efficiency *= 0.9;
      recommendations.push(
        "Business hours work requires coordination with occupants",
      );
    }
    ...
    if (workHours === "after-hours") {
      ...
      efficiency *= 0.95;
      recommendations.push(
        "After-hours work may require security coordination",
      );
    }
  ...
  static calculateMaterialUsage(
    ...
    const supplyCost =
      area *
      baseSupplyRate *
      complexityMultipliers[surfaceComplexity] *
      dustMultipliers[dustLoad];
