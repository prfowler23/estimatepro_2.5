...
export class WindowCleaningCalculator extends BaseCalculator<WindowCleaningInput> {
  calculate(input: WindowCleaningInput): CalculationResult {
    ...
    const accessMethod = this.determineAccessMethod(
    ...
    if (buildingHeightStories > 10 && !hasRoofAnchors) {
      this.addWarning(
        "High-rise building - RDS requires verified roof anchors",
      );
    }

    if (accessMethod.includes("RDS") && !hasRoofAnchors) {
      this.addWarning(
        "RDS access requires roof anchor verification before proceeding",
      );
    }

    if (windows > 200) {
      this.addWarning(
        "Large window count - consider splitting into multiple service visits",
      );
    }
  ...
  static calculateSeasonalAdjustment(basePrice: number, month: number): number {
