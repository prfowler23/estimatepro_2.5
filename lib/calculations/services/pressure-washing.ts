...
export class PressureWashingCalculator extends BaseCalculator<PressureWashingInput> {
  calculate(input: PressureWashingInput): CalculationResult {
    ...
    if (includesHardscapes && hardscapeArea <= 0) {
      throw new Error('Hardscape area must be greater than 0 when hardscapes are included')
    }
    ...
    if (flatSurfaceArea > 0) {
      const flatRate = location === 'charlotte' 
    ...
    const facadeLaborHours = facadeArea / (PRODUCTION_RATES.pwFacade * crewSize)
    const flatLaborHours = flatSurfaceArea / (PRODUCTION_RATES.pwFlatSurface * crewSize)
    const hardscapeLaborHours = hardscapeArea / (PRODUCTION_RATES.pwFlatSurface * crewSize)
    ...
    if (surfaceType === 'ornate' && finalPrice < 1000) {
      this.addWarning('Price seems low for ornate facade - verify measurements')
    }
    ...
    if (surfaceType === 'mixed' && facadeArea > 5000) {
      this.addWarning('Mixed surface type on large area - consider separate pricing for regular vs ornate sections')
    }
  ...
  private getFacadeRate(surfaceType: string): number {
    switch (surfaceType) {
      ...
      case 'mixed':
        return (PRESSURE_WASH_RATES.regularFacade + PRESSURE_WASH_RATES.ornateFacade) / 2
      default:
  ...
  static calculateSurfaceEfficiency(
    ...
    if (facadeRatio < 0.3) {
      return {
        ...
        recommendation: 'Efficient ground-level work - good productivity expected'
      }
    }
  ...
  static validateEnvironmentalConditions(
    ...
    if (temperature < 40) {
      warnings.push('Low temperature - equipment may freeze, consider heated units')
    }
    ...
    if (nearWaterSources) {
      warnings.push('Near water sources - environmental protection measures required')
    }
  ...
  static calculateChemicalUsage(
    ...
    const gallonsNeeded = totalArea * chemicalRates[soilLevel] * surfaceMultiplier
    
    return {
      chemical: 'Multi-Surface Cleaner',
      gallons: Math.ceil(gallonsNeeded),
      cost: gallonsNeeded * 12.50 // $12.50 per gallon
    }
  }
}