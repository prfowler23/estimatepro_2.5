...
export class PressureWashSealCalculator extends BaseCalculator<PressureWashSealInput> {
  calculate(input: PressureWashSealInput): CalculationResult {
    ...
    // Get base rate
    const baseRate =
      PWS_RATES[sealerType === "premium" ? "premiumSealer" : "standardSealer"];
    this.addBreakdown(
      "Base Rate",
      `${sealerType} sealer`,
      `$${baseRate}/sq ft`,
    ...
    if (numberOfCoats > 2) {
      this.addWarning(
        `${numberOfCoats} coats may require extended cure time between applications`,
      );
    }

    if (surfaceMaterial === "mixed" && area > 3000) {
      this.addWarning(
        "Mixed surface type on large area - consider separate pricing for brick vs concrete sections",
      );
    }
    ...
    if (projectDays > 7) {
      this.addWarning(
        "Extended project duration - consider weather delays and cure time",
      );
    }
    ...
    if (surfaceMaterial === "concrete" && numberOfCoats === 1) {
      this.addWarning(
        "Concrete surfaces may benefit from additional coats for optimal protection",
      );
    }
  ...
  static calculateSealerCoverage(
    ...
    const coverage =
      coverageRates[sealerType as keyof typeof coverageRates][
        surfaceMaterial as keyof typeof coverageRates.standard
      ];
  ...
  static validateWeatherConditions(
    ...
    if (temperature < 45 || temperature > 85) {
      warnings.push(
        "Temperature outside optimal range (45-85°F) for sealer application",
      );
    }
