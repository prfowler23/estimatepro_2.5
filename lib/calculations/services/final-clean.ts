...
export class FinalCleanCalculator extends BaseCalculator<FinalCleanInput> {
  calculate(input: FinalCleanInput): CalculationResult {
    ...
    if (windows > 150) {
      this.addWarning('Large window count - consider splitting into multiple days')
    }

    if (postConstruction && finalPrice < 500) {
      this.addWarning('Post-construction cleaning price seems low - verify scope')
    }

    if (includesInterior && buildingHeightStories > 5) {
      this.addWarning('Interior cleaning on high-rise may require additional access arrangements')
    }

    if (projectDays > 3) {
      this.addWarning('Extended final clean duration - consider weather delays')
    }
  ...
  static calculateDetailedCleaningTime(
    ...
    const adjustedTime = baseTimePerWindow * constructionMultipliers[constructionLevel]
  ...
  static calculateMaterialCosts(
    ...
    postConstruction: boolean,
    includesInterior: boolean
  ): { cleaningSupplies: number; protectionMaterials: number; totalCost: number } {
    const baseSupplyCost = windows * 0.50 // $0.50 per window
    const cleaningSupplies = postConstruction ? baseSupplyCost * 1.5 : baseSupplyCost
  ...
  static validateConstructionReadiness(
    ...
    if (includesInterior && constructionPhase !== 'complete') {
      warnings.push('Interior cleaning should wait until construction is complete')
    }
  ...
  static calculateQualityControl(
    ...
    const inspectionTime = postConstruction ? baseInspectionTime * 1.5 : baseInspectionTime
  ...
  static calculateSchedulingRequirements(
    ...
    const weatherBuffer = weatherSensitive ? Math.ceil(recommendedDays * 0.2) : 0
    
    const recommendations: string[] = []
    
    if (includesInterior) {
      recommendations.push('Schedule interior cleaning after exterior completion')
    }
