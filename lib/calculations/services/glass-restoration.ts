...
export class GlassRestorationCalculator extends BaseCalculator<GlassRestorationInput> {
  calculate(input: GlassRestorationInput): CalculationResult {
    ...
    if (windows > 500) {
      this.addWarning(
        "Large window count - consider splitting into multiple phases",
      );
    }

    if (numberOfDrops > 6) {
      this.addWarning(
        "High number of drops - verify scaffold rigging requirements",
      );
    }

    if (projectDays > 30) {
      this.addWarning(
        "Project duration exceeds 30 days - consider weather delays",
      );
    }
  ...
  static validateWeatherConditions(
    ...
    if (temperature < 35) {
      warnings.push(
        "Temperature below 35°F - Glass Restoration not recommended",
      );
    }
  ...
  static calculateCombinedEfficiency(
    ...
    if (servicesIncluded.includes("FR")) {
      ...
      savings += totalGlassArea * 0.5; // $0.50 per sq ft savings
    }

    // Final Clean combination
    if (servicesIncluded.includes("FC")) {
      efficiency *= 0.9; // 10% efficiency gain
