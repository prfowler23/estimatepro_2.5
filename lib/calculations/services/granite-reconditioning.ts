...
import { SERVICE_RATES, EQUIPMENT_TYPES } from "../constants";
import { roundToNearest, validateBuildingMeasurements } from "../utils";
...
export class GraniteReconditioningCalculator extends BaseCalculator<GraniteReconditioningInput> {
  calculate(input: GraniteReconditioningInput): CalculationResult {
    ...
    if (edgeWork && edgeLinearFeet <= 0) {
      throw new Error(
        "Edge linear feet must be greater than 0 when edge work is included",
      );
    }
    ...
    // Edge work
    if (edgeWork && edgeLinearFeet > 0) {
      const edgeRate = 3.5; // $3.50 per linear foot
    ...
    if (graniteCondition === "poor" && serviceLevel === "clean_only") {
      this.addWarning(
        "Poor condition granite typically requires restoration, not just cleaning",
      );
    }

    if (area > 1000) {
      this.addWarning(
        "Large area - consider phased approach to minimize business disruption",
      );
    }

    if (includesPolishing && graniteCondition === "poor") {
      this.addWarning(
        "Polishing may not be effective on poor condition granite without restoration",
      );
    }

    if (serviceLevel === "restore_and_seal" && finalPrice < 1000) {
      this.addWarning(
        "Restoration price seems low - verify scope and measurements",
      );
    }
    ...
    if (graniteCondition === "good" && serviceLevel === "restore_and_seal") {
      this.addWarning(
        "Full restoration may not be necessary for good condition granite",
      );
    }
    ...
    if (serviceLevel.includes("seal") && graniteCondition === "poor") {
      this.addWarning(
        "Poor condition granite may require multiple sealer applications",
      );
    }
  ...
  static calculateMaterialRequirements(
    ...
    if (serviceLevel.includes("seal")) {
      ...
      if (graniteCondition === "poor") {
        recommendations.push(
          "Poor condition granite may require multiple sealer coats",
        );
      }
    }
    ...
    if (includesPolishing) {
      ...
      recommendations.push(
        "Polishing requires diamond pads and specialized equipment",
      );
    }
    ...
    // Calculate costs
    const costs = {
      cleaner: cleaner * 18.5, // $18.50 per gallon
      sealer: sealer * 45.0, // $45.00 per gallon
      polishingCompound: polishingCompound * 35.0, // $35.00 per gallon
      diamondPads: diamondPads * 25.0, // $25.00 per pad
    };

    const totalCost =
      costs.cleaner +
      costs.sealer +
      costs.polishingCompound +
      costs.diamondPads;
  ...
  static calculateRestorationTimeline(
    ...
    if (serviceLevel === "restore_and_seal") {
      recommendations.push(
        "Restoration work should be done in controlled environment",
      );
    }

    if (includesPolishing) {
      recommendations.push(
        "Polishing generates dust - ensure proper ventilation",
      );
    }
    ...
    if (graniteCondition === "poor") {
      recommendations.push(
        "Poor condition granite may require extended timeline",
      );
    }
    ...
    const cleaningCost =
      ...
      (cleaningFrequency === "weekly"
        ...
        : cleaningFrequency === "bi-weekly"
          ...
          : 12);
    const sealingCost =
      area * 1.75 * (sealingFrequency === "every 6 months" ? 2 : 1);
    const polishingCost =
      area * 0.25 * (polishingFrequency === "annually" ? 1 : 0.5);
