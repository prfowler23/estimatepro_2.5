...
export class FrameRestorationCalculator extends BaseCalculator<FrameRestorationInput> {
  calculate(input: FrameRestorationInput): CalculationResult {
    ...
    if (!requiresGlassRestoration) {
      this.addWarning(
        "Frame restoration typically performed with glass restoration for efficiency",
      );
    }

    if (frameCondition === "poor" && numberOfFrames > 100) {
      this.addWarning(
        "Large number of frames in poor condition - consider phased approach",
      );
    }

    if (numberOfFrames > 500) {
      this.addWarning(
        "Very large frame count - consider splitting into multiple phases",
      );
    }

    if (frameCondition === "poor" && finalPrice < 1000) {
      this.addWarning(
        "Price seems low for poor condition frames - verify scope",
      );
    }
  ...
  static assessFrameCondition(
    ...
    if (frameAge > 20) {
      ...
      recommendations.push(
        "Frames over 20 years old require careful inspection",
      );
    }
    ...
    if (frameAge > 30) {
      condition = "poor";
      recommendations.push(
        "Frames over 30 years old may need significant restoration",
      );
    }
    ...
    if (lastMaintenance > 10) {
      ...
      recommendations.push(
        "Frames not maintained in 10+ years require additional work",
      );
    }
    ...
    if (environmentalExposure === "high") {
      condition = condition === "good" ? "fair" : "poor";
      recommendations.push(
        "High environmental exposure accelerates frame deterioration",
      );
    }
  ...
  static calculateRestorationTimeline(
    ...
    const adjustedTime =
      baseTimePerFrame * conditionMultipliers[frameCondition];
    ...
    if (frameCondition === "poor") {
      recommendations.push(
        "Poor condition frames may require additional prep time",
      );
    }
  ...
  static validateEnvironmentalConditions(
    ...
    if (temperature < 50 || temperature > 90) {
      warnings.push(
        "Temperature outside optimal range (50-90°F) for frame restoration",
      );
    }
    ...
    if (precipitationChance > 30) {
      warnings.push(
        "Precipitation risk - frame restoration requires dry conditions",
      );
    }
  ...
  static calculateCombinedEfficiency(
    ...
    const sharedSetupCost = Math.min(
      standaloneFrameCost,
      sharedScaffoldDays * 100,
    );

    const savings = standaloneFrameCost - sharedSetupCost;
    const efficiency =
      1 + savings / (numberOfFrames * SERVICE_RATES.frameRestoration);
