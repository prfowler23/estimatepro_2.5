...
import { SERVICE_RATES, SURFACE, EQUIPMENT_TYPES } from "../constants";
...
export class ParkingDeckCalculator extends BaseCalculator<ParkingDeckInput> {
  calculate(input: ParkingDeckInput): CalculationResult {
    ...
    if (deckLevel === "underground") {
      ...
      const elevatedSurcharge = basePrice * 0.1; // 10% increase
    ...
    if (spaces! > 500) {
      this.addWarning(
        "Large parking deck - consider scheduling during off-peak hours",
      );
    }

    if (deckLevel === "underground" && serviceType.includes("wash")) {
      this.addWarning(
        "Underground washing requires special ventilation considerations",
      );
    }

    if (hasOilStains && serviceType === "sweep_only") {
      this.addWarning(
        "Oil stains typically require washing service for effective removal",
      );
    }

    if (drainageComplexity === "complex" && serviceType.includes("wash")) {
      this.addWarning(
        "Complex drainage may require additional containment measures",
      );
    }

    if (spaces! > 1000) {
      this.addWarning(
        "Very large parking deck - consider splitting into multiple phases",
      );
    }
    ...
    if (projectDays > 3) {
      this.addWarning(
        "Extended project duration - coordinate with facility management",
      );
    }
  ...
  private getServiceRate(
    serviceType: string,
    locationRates: { min: number; max: number },
  ): number {
    switch (serviceType) {
      case "sweep_only":
    ...
    if (serviceType.includes("wash")) {
      ...
      if (deckLevel === "underground") {
        ...
        recommendations.push(
          "Underground washing requires additional ventilation",
        );
      }
    }

    if (hasOilStains) {
      oilTreatment = spaces * 0.5; // $0.50 per space for oil treatment
      recommendations.push(
        "Oil stain treatment requires specialized chemicals",
      );
    }
  ...
  static calculateEnvironmentalImpact(
    ...
    const environmentalScore =
      waterUsage * 0.1 + chemicalUsage * 2 + wasteGenerated * 0.5;
  ...
  static calculateTrafficManagement(
    ...
    if (workingHours === "business") {
      recommendations.push(
        "Business hours work requires coordination with users",
      );
      if (spaces > 100) {
