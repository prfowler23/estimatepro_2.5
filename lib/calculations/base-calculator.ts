...
export abstract class BaseCalculator<T extends CalculationInput> {
  ...
  protected addBreakdown(step: string, description: string, value: string | number): void {
    this.breakdown.push({ step, description, value })
  }

  protected calculateSetupTime(laborHours: number, isSpecial: boolean = false): number {
    if (isSpecial) {
  ...
  protected validateGlassToFacadeRatio(glassArea: number, facadeArea: number): void {
    if (facadeArea > 0) {
  ...
  protected validatePricePerWindow(totalPrice: number, windowCount: number): void {
    if (windowCount > 0) {
      const pricePerWindow = totalPrice / windowCount
      if (pricePerWindow < VALIDATION.pricePerWindowRange.min || 
  ...
  protected validateSetupTime(setupHours: number, laborHours: number): void {
    ...
    if (setupPercentage < VALIDATION.minSetupTimePercent || 
  ...
  protected determineEquipmentType(buildingHeight: number, serviceType: string): string {
  ...
  protected calculateMinimumCharge(serviceType: string, calculatedPrice: number): number {
    const minimums = {
      'GR': 500,
      'WC': 150,
      'PW': 200,
  ...
  protected buildCalculationResult(
    ...
    const projectDays = this.calculateProjectDays(totalHours, crewSize, this.getShiftLength())
