...
export class DurationCalculationService {
  calculateServiceDuration(
    ...
    const totalQuantity = this.getTotalQuantityForService(
      service,
      measurements,
    );
  ...
  getTotalQuantityForService(
    service: string,
    measurements: MeasurementEntry[],
  ): number {
    const relevantMeasurements = this.getRelevantMeasurementsForService(
      service,
      measurements,
    );
    const rate = PRODUCTION_RATES[service];
  ...
  private getRelevantMeasurementsForService(
    service: string,
    measurements: MeasurementEntry[],
  ): MeasurementEntry[] {
    switch (service) {
      ...
      case "BWS":
      case "HBW":
        return measurements.filter((m) => m.category.startsWith("facade_"));

      case "PWF":
      case "HFS":
        return measurements.filter((m) => m.category === "flat_surface");

      case "PC":
        return measurements.filter((m) => m.category === "parking_spaces");

      case "PWP":
        return measurements.filter(
          (m) =>
            m.category === "parking_deck" || m.category === "parking_spaces",
        );

      case "IW":
        return measurements.filter((m) => m.category === "inner_wall");

      case "DC":
        return measurements.filter((m) => m.category === "ceiling");
  ...
  private determineConfidence(
    ...
    const hasDetailedMeasurements = relevantMeasurements.every(
      (m) =>
        m.description &&
        m.description.trim() !== "" &&
        m.width > 0 &&
    ...
    const hasReasonableValues = relevantMeasurements.every(
      (m) => m.total > 0 && m.total < 50000, // Reasonable range
    );

    if (
      hasDetailedMeasurements &&
      hasReasonableValues &&
      relevantMeasurements.length >= 3
    ) {
      return "high";
  ...
  scheduleServices(
    ...
    sortedServices.forEach((duration) => {
      ...
      if (!allowOverlaps || !this.canOverlapWithOthers(duration.service)) {
        ...
        const nonOverlappingEnd = this.findLatestNonOverlappingEnd(
          duration.service,
          timeline,
        );
      ...
      const endDate = this.addBusinessDays(
        actualStartDate,
  ...
  private topologicalSort(
    ...
    const visit = (service: ServiceDuration) => {
      ...
      service.dependencies.forEach((depServiceId) => {
        const depService = serviceDurations.find(
          (s) => s.service === depServiceId,
        );
        if (depService) {
          visit(depService);
        }
  ...
  private canOverlapWithOthers(service: string): boolean {
    return (
      Object.keys(OVERLAPPING_SERVICES).includes(service) ||
      Object.values(OVERLAPPING_SERVICES).some((services) =>
        services.includes(service),
      )
    );
  }

  private findLatestNonOverlappingEnd(
    service: string,
    ...
    timeline.forEach((entry) => {
      if (
        !overlappingServices.includes(entry.service) &&
  ...
  canServicesOverlap(service1: string, service2: string): boolean {
    ...
    return (
      service1Overlaps.includes(service2) || service2Overlaps.includes(service1)
    );
  }

  // Method to estimate crew requirements
  estimateCrewRequirements(
    serviceDurations: ServiceDuration[],
  ...
  calculateResourceUtilization(
    timeline: Timeline,
  ): Array<{ date: Date; crewsNeeded: number; services: string[] }> {
    ...
    const startDate = new Date(
      Math.min(...timeline.entries.map((e) => e.startDate.getTime())),
    );
    const endDate = new Date(
      Math.max(...timeline.entries.map((e) => e.endDate.getTime())),
    );
    ...
    while (current <= endDate) {
      if (current.getDay() !== 0 && current.getDay() !== 6) {
        // Business days only
        const activeServices = timeline.entries.filter(
          (entry) => current >= entry.startDate && current <= entry.endDate,
        );
