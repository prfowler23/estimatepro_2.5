...
export class CalendarExportService {
  ...
  exportToICS(
    ...
    timeline.entries.forEach((entry) => {
      const serviceEvents = this.createServiceEvents(
        entry,
        projectInfo,
        includeWeekends,
      );
    ...
    if (timeline.criticalPath.length > 0) {
      const criticalPathEvent = this.createCriticalPathEvent(
        timeline,
        projectInfo,
      );
  ...
  private createServiceEvents(
    ...
    if (entry.duration > 1) {
      ...
      if (
        includeWeekends ||
        (prepDate.getDay() !== 0 && prepDate.getDay() !== 6)
      ) {
        const prepEvent = this.createPreparationEvent(
          entry,
          projectInfo,
          prepDate,
        );
    ...
    if (entry.duration > 3) {
      const reminderEvents = this.createDailyReminders(
        entry,
        projectInfo,
        includeWeekends,
      );
  ...
  private createEvent(
    entry: TimelineEntry,
    projectInfo: ProjectInfo,
    type: "service" | "milestone" | "reminder",
  ): string[] {
    ...
    const serviceName =
      this.SERVICE_NAMES[entry.service] || entry.serviceName || entry.service;
   ...
   */
  private createPreparationEvent(
    entry: TimelineEntry,
    projectInfo: ProjectInfo,
    prepDate: Date,
  ): string[] {
   ...
   */
  private createCriticalPathEvent(
    timeline: Timeline,
    projectInfo: ProjectInfo,
  ): string[] {
    ...
    const endDate =
      timeline.entries[timeline.entries.length - 1]?.endDate || new Date();

    const description =
      `Critical Path Services: ${timeline.criticalPath.join(", ")}\\n\\n` +
  ...
  private createDailyReminders(
    ...
    while (current < end) {
      ...
      if (current < end) {
        dayCount++;
        const serviceName =
          this.SERVICE_NAMES[entry.service] ||
          entry.serviceName ||
          entry.service;
  ...
  exportToGoogleCalendar(
    ...
    const details = encodeURIComponent(
      this.buildDescription(entry, projectInfo).replace(/\\n/g, "\n"),
    );
   ...
   */
  validateTimelineForExport(timeline: Timeline): {
    valid: boolean;
    errors: string[];
  } {
    const errors: string[] = [];
