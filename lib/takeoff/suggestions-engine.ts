...
export class TakeoffSuggestionsEngine {
  getSuggestionsForBuildingType(
    ...
    templates.forEach(template => {
      ...
      if (isRequired) {
        const exists = existingMeasurements.some(m => 
    ...
    this.addServiceSpecificSuggestions(services, existingMeasurements, suggestions);
    ...
    return suggestions.sort((a, b) => this.getPriorityWeight(a.priority) - this.getPriorityWeight(b.priority));
  }
  ...
  private addServiceSpecificSuggestions(
    ...
    if (services.includes('WC') || services.includes('GR')) {
      const windowCount = existingMeasurements.filter(m => 
      ...
      if (windowCount === 0) {
        suggestions.push({
          ...
          message: 'Window measurements required for Window Cleaning/Glass Restoration',
          priority: 'critical',
      ...
      const doorCount = existingMeasurements.filter(m => 
      ...
      if (doorCount === 0) {
        suggestions.push({
          ...
          message: 'Consider adding glass door measurements if building has glass entrances',
          priority: 'medium',
    ...
    // Pressure Washing
    if (services.includes('BWP') || services.includes('BWS') || services.includes('HBW')) {
      const facadeArea = existingMeasurements.filter(m => 
      ...
      if (facadeArea === 0) {
        suggestions.push({
          ...
          message: 'Facade measurements required for Building Washing/Pressure Washing',
          priority: 'critical',
          template: { width: 50, height: 40, quantity: 1 },
          reason: 'Cannot estimate pressure washing without facade area measurements'
        });
      }
    }
    ...
    if (services.includes('PWF') || services.includes('HFS')) {
      const flatSurfaceArea = existingMeasurements.filter(m => 
      ...
      if (flatSurfaceArea === 0) {
        suggestions.push({
          ...
          message: 'Flat surface measurements required for sidewalk/plaza cleaning',
          priority: 'critical',
          template: { width: 20, height: 100, quantity: 1 },
          reason: 'Cannot estimate flat surface cleaning without area measurements'
        });
      }
    }
    ...
    if (services.includes('PC') || services.includes('PWP')) {
      ...
      if (parkingMeasurements.length === 0) {
        suggestions.push({
          ...
          message: 'Parking measurements required for parking cleaning services',
          priority: 'critical',
          template: { width: 9, height: 18, quantity: 50 },
          reason: 'Cannot estimate parking cleaning without space or deck measurements'
        });
      }
    }
    ...
    if (services.includes('IW')) {
      const innerWallArea = existingMeasurements.filter(m => 
      ...
      if (innerWallArea === 0) {
        suggestions.push({
          category: 'inner_wall',
          message: 'Interior wall measurements required for inner wall cleaning',
          priority: 'critical',
          template: { width: 20, height: 10, quantity: 1 },
          reason: 'Cannot estimate interior wall cleaning without wall area measurements'
        });
      }
    }
    ...
    if (services.includes('DC')) {
      const ceilingArea = existingMeasurements.filter(m => 
        m.category === 'ceiling'
      ).reduce((sum, m) => sum + m.total, 0);
      ...
      if (ceilingArea === 0) {
        suggestions.push({
          category: 'ceiling',
          message: 'Ceiling/deck measurements required for deck cleaning service',
          priority: 'critical',
          template: { width: 30, height: 40, quantity: 1 },
          reason: 'Cannot estimate deck cleaning without ceiling/deck area measurements'
        });
      }
    }
  }
  ...
  private addQualityCheckSuggestions(
    ...
    const zeroMeasurements = existingMeasurements.filter(m => 
      m.width === 0 || m.height === 0 || m.total === 0
    );
    ...
    if (zeroMeasurements.length > 0) {
      suggestions.push({
        ...
        priority: 'high',
        reason: 'Zero-dimension measurements will result in inaccurate estimates'
      });
    }
    
    // Check for missing descriptions
    const missingDescriptions = existingMeasurements.filter(m => 
    ...
    const missingLocations = existingMeasurements.filter(m => 
  ...
  private addEfficiencySuggestions(
    ...
    const categoryGroups = existingMeasurements.reduce((groups, measurement) => {
    ...
    Object.entries(categoryGroups).forEach(([category, measurements]) => {
      if (measurements.length > 10) {
        suggestions.push({
          ...
          priority: 'low',
          reason: 'Too many small measurements can slow down estimation and invoicing'
        });
      }
    });
    ...
    const hasFootprint = existingMeasurements.some(m => m.category === 'footprint');
    
    if (totalFacadeArea > 10000 && !hasFootprint) {
      suggestions.push({
        category: 'footprint',
        message: 'Consider adding building footprint measurement for large building projects',
        priority: 'medium',
        template: { width: 100, height: 150, quantity: 1 },
        reason: 'Building footprint helps with project planning and equipment staging'
      });
    }
  }
  ...
  getPhotoAnalysisSuggestions(photoAnalysis: any, existingMeasurements: MeasurementEntry[]): Suggestion[] {
    const suggestions: Suggestion[] = [];
    ...
    if (photoAnalysis.detectedFeatures) {
      ...
      if (windows && windows.count > 0) {
        const existingWindows = existingMeasurements.filter(m => m.category === 'glass_windows').length;
        if (existingWindows < windows.count) {
          suggestions.push({
            category: 'glass_windows',
            message: `Photo analysis detected ${windows.count} windows, but only ${existingWindows} measured`,
            priority: 'high',
            template: { width: windows.avgWidth || 4, height: windows.avgHeight || 6, quantity: windows.count - existingWindows }
          });
        }
      }
      ...
      if (doors && doors.count > 0) {
        const existingDoors = existingMeasurements.filter(m => m.category === 'glass_doors').length;
        if (existingDoors < doors.count) {
          suggestions.push({
            category: 'glass_doors',
            message: `Photo analysis detected ${doors.count} doors, consider adding measurements`,
            priority: 'medium',
            template: { width: doors.avgWidth || 3, height: doors.avgHeight || 7, quantity: doors.count - existingDoors }
          });
        }
      }
      ...
      if (materials) {
        materials.forEach((material: any) => {
          ...
          const existingMaterial = existingMeasurements.some(m => m.category === category);
          ...
          if (!existingMaterial && material.confidence > 0.7) {
            suggestions.push({
              ...
              priority: 'medium',
              template: { width: material.estimatedWidth || 20, height: material.estimatedHeight || 15, quantity: 1 }
            });
          }
        });
      }
