  ...
  parking_spaces: {
    ...
    fields: [
      ...
      { name: "quantity", type: "number", required: true },
      {
        ...
        type: "select",
        required: true,
        options: ["standard", "compact", "handicap", "motorcycle"],
      },
    ],
    calculation: "count",
  },
...
export function calculateMeasurement(
  ...
  switch (template.calculation) {
    ...
    case "volume":
      return (
        values.width * values.height * values.length * (values.quantity || 1)
      );
    default:
      return 0;
  }
...
export function getUnitForCalculation(
  calculation: MeasurementTemplate["calculation"],
): MeasurementEntry["unit"] {
  switch (calculation) {
    case "area":
      return "sqft";
    case "linear":
...
export function getRelevantCategories(
  ...
  Object.values(MEASUREMENT_TEMPLATES).forEach((template) => {
    if (
      template.requiredFor.some((serviceId) =>
...
): Record<string, MeasurementCategory[]> {
