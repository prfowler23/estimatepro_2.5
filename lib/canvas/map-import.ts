...
export class MapImportService {
   ...
   */
  private async detectGoogleEarthScale(imageUrl: string): Promise<ScaleDetectionResult> {
    const img = await this.loadImage(imageUrl);
    this.canvas.width = img.width;
    this.canvas.height = img.height;
    this.ctx.drawImage(img, 0, 0);
    ...
    const scaleBarRegion = this.ctx.getImageData(
      0, 
      img.height - 100, 
      300, 
      100
    ...
    const result = this.analyzeScaleBarRegion(scaleBarRegion, img.width, img.height);
   ...
   */
  private async detectScaleBar(img: HTMLImageElement): Promise<ScaleDetectionResult> {
   ...
   */
  private async detectGridPattern(img: HTMLImageElement): Promise<ScaleDetectionResult> {
   ...
   */
  private async detectRulerElements(img: HTMLImageElement): Promise<ScaleDetectionResult> {
   ...
   */
  private async detectTextualScale(img: HTMLImageElement): Promise<ScaleDetectionResult> {
   ...
   */
  private async extractMeasurementsFromPDF(file: File): Promise<Array<{
    value: number;
    unit: string;
   ...
   */
  private analyzeScaleBarRegion(imageData: ImageData, imgWidth: number, imgHeight: number): {
    found: boolean;
    pixelsPerFoot?: number;
    confidence?: number;
  } {
