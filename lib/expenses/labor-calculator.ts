...
export const LABOR_ROLES: LaborRole[] = [
  {
    ...
    description: 'Experienced supervisor responsible for crew coordination and safety',
    skillLevel: 'advanced'
  },
  {
    id: 'technician',
  ...
  },
  {
    ...
    overtimeRate: 57,
    benefitsRate: 55,
    requiredCertifications: ['OSHA-30', 'Rope Access Level 2', 'Fall Protection'],
...
export class LaborCalculator {
  ...
  calculateLaborCosts(
    ...
    services.forEach(serviceDuration => {
      const requirement = CREW_REQUIREMENTS.find(r => r.service === serviceDuration.service);
      ...
      requirement.roles.forEach(({ roleId, count, efficiency = 1.0 }) => {
        ...
        const adjustedDuration = serviceDuration.finalDuration * productivityFactors.totalMultiplier;
        const totalHours = adjustedDuration * requirement.hoursPerDay * count * efficiency;
        ...
        const benefitsCost = (baseCost + overtimeCost) * (role.benefitsRate / 100);
        ...
        costs.push({
          ...
          notes: this.generateLaborNotes(role, requirement, productivityFactors, buildingHeight)
        });
      });
    });
  ...
  private calculateProductivityFactors(
    ...
    const heightMultiplier = buildingHeight > 3 ? 
      1 + ((buildingHeight - 3) * 0.05) : 1.0;
    
    // Weather impact adjustment
    const weatherMultiplier = 1 + (weatherImpact * requirement.productivityFactors.weather);
    ...
    const complexityMultiplier = complexityMultipliers[requirement.productivityFactors.complexity];
    ...
    const totalMultiplier = heightMultiplier * weatherMultiplier * complexityMultiplier * experienceMultiplier;
  ...
  calculateTotalLaborCosts(laborCosts: LaborCost[]): {
    ...
    const overtime = laborCosts.reduce((sum, cost) => sum + cost.overtimeCost, 0);
    const benefits = laborCosts.reduce((sum, cost) => sum + cost.benefitsCost, 0);
    
    const byRole = laborCosts.reduce((acc, cost) => {
      acc[cost.role] = (acc[cost.role] || 0) + cost.totalCost;
      return acc;
    }, {} as Record<string, number>);
    
    const byService = laborCosts.reduce((acc, cost) => {
   ...
   */
  estimateLaborHours(services: string[], totalDuration: number): {
    totalHours: number;
    crewSize: number;
    averageRate: number;
  } {
    ...
    services.forEach(service => {
      ...
      if (requirement) {
        ...
        const serviceWorkers = requirement.roles.reduce((sum, role) => sum + role.count, 0);
