...
export class EquipmentSelector {
  ...
  selectRequiredEquipment(
    ...
    try {
      const dynamicEquipment = EquipmentMaterialsService.getEquipment({
        available: true,
      });

      if (dynamicEquipment.length > 0) {
        return this.selectFromDynamicEquipment(
          dynamicEquipment,
          services,
          duration,
          buildingHeight,
      ...
      console.warn(
        "Failed to load dynamic equipment data, using static fallback:",
        error,
      );
    }
    ...
    const requiredEquipment = EQUIPMENT_DATABASE.filter(
      (eq) =>
    ...
    requiredEquipment.forEach((eq) => {
      ...
      if (!existing) {
        ...
        if (
          eq.category === "lift" &&
          eq.specifications.reach &&
          existing.specifications.reach
        ) {
          if (
            buildingHeight >= 5 &&
            eq.specifications.reach > existing.specifications.reach
          ) {
            byCategory.set(eq.category, eq);
          }
        }
    ...
    byCategory.forEach((equipment) => {
      ...
      const { period, rate, totalCost } = this.calculateOptimalRental(
        equipment,
        rentalDays,
      );
  ...
  /**
   * Select equipment from dynamic service data
   */
  private selectFromDynamicEquipment(
    dynamicEquipment: any[],
    services: string[],
    duration: number,
    buildingHeight: number,
  ): EquipmentSelection[] {
    ...
    // Convert dynamic equipment to our format for processing
    const convertedEquipment = dynamicEquipment.map((eq) =>
      this.convertDynamicToStatic(eq),
    );

    // Use existing logic with converted data
    const requiredEquipment = convertedEquipment.filter(
      (eq) =>
        eq.requiredFor.some((service) => services.includes(service)) &&
        (!eq.heightRequirement || buildingHeight >= eq.heightRequirement),
    );
    ...
    byCategory.forEach((equipment) => {
      const rentalDays = Math.ceil(duration * 1.1);
      const { period, rate, totalCost } = this.calculateOptimalRental(
        equipment,
        rentalDays,
      );
  ...
  /**
   * Convert dynamic equipment data to static format
   */
  private convertDynamicToStatic(dynamicEq: any): Equipment {
    return {
      id: dynamicEq.id,
      name: dynamicEq.name,
      category: this.mapDynamicCategory(dynamicEq.category),
      dailyRate: dynamicEq.dailyRate,
      weeklyRate: dynamicEq.weeklyRate || dynamicEq.dailyRate * 4.5,
      monthlyRate: dynamicEq.monthlyRate || dynamicEq.dailyRate * 18,
      requiredFor: this.mapServicesFromCategory(dynamicEq.category),
      heightRequirement: this.getHeightRequirement(dynamicEq),
      specifications: dynamicEq.specifications || {},
      vendors: dynamicEq.vendors.map((v: any) => ({
        vendorId: v.id,
        dailyRate: v.dailyRate || dynamicEq.dailyRate,
        weeklyRate: v.weeklyRate,
        monthlyRate: v.monthlyRate,
        available: true,
        leadTime: 1,
        deliveryCharge: v.deliveryFee,
        minRentalPeriod: v.minimumRentalDays || 1,
      })),
      description: dynamicEq.description,
      alternatives: [],
    };
  }

  /**
   * Map dynamic categories to static categories
   */
  private mapDynamicCategory(category: string): Equipment["category"] {
    const categoryMap: Record<string, Equipment["category"]> = {
      Lifts: "lift",
      "Pressure Equipment": "cleaning",
      "Cleaning Equipment": "cleaning",
      "Safety Equipment": "safety",
      "Power Equipment": "power",
      "Transport Equipment": "transport",
    };
    return categoryMap[category] || "specialized";
  }

  /**
   * Map equipment category to required services
   */
  private mapServicesFromCategory(category: string): string[] {
    const serviceMap: Record<string, string[]> = {
      Lifts: ["WC", "GR", "BWP", "BWS", "HBW"],
      "Pressure Equipment": ["BWP", "BWS", "PWF", "PWP", "DC"],
      "Cleaning Equipment": ["HFS", "PC", "WC"],
      "Safety Equipment": ["WC", "GR", "BWP", "BWS", "HBW"],
      "Power Equipment": ["HFS", "PC", "IW"],
      "Transport Equipment": ["BWP", "BWS", "PWF", "PWP"],
    };
    return serviceMap[category] || ["WC"];
  }

  /**
   * Get height requirement from specifications
   */
  private getHeightRequirement(equipment: any): number | undefined {
    const specs = equipment.specifications || {};
    const reach = specs.maxHeight || specs.reach;
    if (reach) {
      const height = parseInt(reach.toString());
      if (height >= 60) return 5;
      if (height >= 40) return 3;
      if (height >= 26) return 2;
      return 1;
    }
    return undefined;
  }
  ...
  private generateEquipmentNotes(
    ...
    if (
      equipment.heightRequirement &&
      buildingHeight >= equipment.heightRequirement
    ) {
  ...
  searchEquipment(criteria: {
    ...
    return EQUIPMENT_DATABASE.filter((eq) => {
      ...
      if (criteria.service && !eq.requiredFor.includes(criteria.service))
        return false;
      if (criteria.maxDailyRate && eq.dailyRate > criteria.maxDailyRate)
        return false;
      if (
        criteria.heightRequirement &&
        eq.heightRequirement &&
        eq.heightRequirement > criteria.heightRequirement
      )
        return false;
      return true;
    });
  ...
  calculateTotalEquipmentCost(selections: EquipmentSelection[]): {
    ...
    const breakdown = selections.reduce(
      (acc, selection) => {
        acc[selection.equipment.category] =
          (acc[selection.equipment.category] || 0) + selection.totalCost;
        return acc;
      },
