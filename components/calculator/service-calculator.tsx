    ...
    <div className='space-y-8'>
          ...
          return (
            <Card 
              ...
              className={`cursor-pointer transition-all hover:scale-105 hover:shadow-lg hover:border-border-focus active:scale-[1.02] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-action focus-visible:ring-offset-2 ${
                isSelected ? 'ring-2 ring-primary-action bg-primary-action/5' : ''
              }`}
            ...
            >
              <CardHeader className='pb-3'>
                <div className='flex items-center justify-between'>
                  ...
                  {service.popular && (
                    <Badge variant='secondary' className='bg-feedback-success text-white'>Popular</Badge>
                  )}
                  {isSelected && (
              ...
              <CardContent>
                <div className='flex items-center justify-between'>
                  <Badge variant='outline' className='border-text-secondary text-text-secondary'>{service.pricing}</Badge>
      ...
      {services.length > 0 && (
        <Card>
          ...
          <CardContent className='space-y-4'>
            {services.map((service) => {
              const serviceInfo = SERVICES.find(s => s.id === service.serviceType)
              return (
            ...
            <div className='space-y-2'>
              ...
              {totalEquipmentCost > 0 && (
                <div className='flex justify-between text-lg'>
                  ...
                  <span className='font-bold'>${totalEquipmentCost.toLocaleString()}</span>
                </div>
              )}
              ...
              <div className='flex justify-between text-xl font-bold text-primary'>
                ...
                <span>${(totalPrice + totalEquipmentCost).toLocaleString()}</span>
              </div>
            </div>
            ...
            {services.some(s => s.calculationResult.warnings?.length > 0) && (
              <Alert>
                ...
                <AlertDescription>
                  Some services have warnings. Review individual calculations for details.
                </AlertDescription>
              </Alert>
            )}
            ...
            <div className='flex gap-2 pt-4'>
              ...
              >
                {isSaving ? 'Saving...' : user ? 'Save Quote' : 'Sign In to Save'}
              </Button>
      ...
      <Dialog open={isModalOpen} onOpenChange={(open) => {
        ...
        <DialogContent className='max-w-4xl max-h-[90vh] overflow-y-auto'>
          <DialogHeader>
            <DialogTitle>
              {currentService && SERVICES.find(s => s.id === currentService)?.name} Calculator
            </DialogTitle>
          </DialogHeader>
