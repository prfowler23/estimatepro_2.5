...
import { windowCleaningSchema, type WindowCleaningFormData } from '@/lib/schemas/service-forms'
...
export function WindowCleaningForm({ onSubmit, onCancel }: WindowCleaningFormProps) {
  ...
  const estimatedWindows = watchedValues.glassArea ? Math.ceil(watchedValues.glassArea / 24) : 0
  const hourlyRate = watchedValues.location === 'charlotte' ? 65 : 75
  const accessMethod = getAccessMethod(watchedValues.buildingHeightStories, watchedValues.hasRoofAnchors)

  function getAccessMethod(heightStories: number, hasRoofAnchors: boolean): string {
          ...
          <form onSubmit={form.handleSubmit(handleSubmit)} className='space-y-6'>
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <Select onValueChange={field.onChange} defaultValue={field.value}>
                          ...
                          <SelectContent>
                            <SelectItem value='raleigh'>Raleigh ($75/hr)</SelectItem>
                            <SelectItem value='charlotte'>Charlotte ($65/hr)</SelectItem>
                            <SelectItem value='greensboro'>Greensboro ($75/hr)</SelectItem>
                          </SelectContent>
                        </Select>
                        <FormMessage />
                      </FormItem>
                ...
                <CardContent>
                  ...
                  {!watchedValues.hasRoofAnchors && (
                    <Alert className='mt-4'>
                      ...
                      <AlertDescription>
                        High-rise buildings require verified roof anchors for safe RDS access. 
                        Alternative access methods may increase cost.
                      </AlertDescription>
                    </Alert>
                  )}
                </CardContent>
            ...
            <div className='flex justify-end space-x-4'>
              ...
              <Button type='submit' disabled={!calculation || isCalculating}>
                {isCalculating ? 'Calculating...' : 'Add to Quote'}
              </Button>
            </div>
          </form>
        </Form>
          ...
          <Card>
            <CardHeader>
              <CardTitle className='flex items-center justify-between'>
                <span>Window Cleaning Quote</span>
                <Badge variant='secondary'>${calculation.basePrice.toLocaleString()}</Badge>
              </CardTitle>
            </CardHeader>
            <CardContent className='space-y-4'>
              {/* Quick Stats */}
              <div className='grid grid-cols-2 gap-4 p-4 bg-muted rounded-lg'>
                <div className='text-center'>
                  <p className='text-2xl font-bold text-feedback-success'>{estimatedWindows}</p>
                  <p className='text-sm text-muted-foreground'>Windows</p>
                </div>
                <div className='text-center'>
                  <p className='text-2xl font-bold text-primary-action'>${hourlyRate}/hr</p>
                ...
                <>
                  <div className='pt-4 border-t'>
                    ...
                    <div className='flex justify-between'>
                      ...
                      <span>${calculation.equipment.cost.toLocaleString()}</span>
                    </div>
                  </div>
                </>
              )}
