...
export function Pricing({ data, onUpdate, onNext, onBack }: { data: any; onUpdate: (data: any) => void; onNext: () => void; onBack: () => void }) {
  ...
  const calculateOptimalPricing = async () => {
    ...
    const factors = {
      ...
      margins: data.expenses?.margins || { equipment: 15, materials: 20, labor: 35 },
      marketData,
      customerProfile: {
        companySize: customerProfile.customer?.company ? 'medium' : 'small',
        timeline: customerProfile.timeline || {},
        priority: customerProfile.timeline?.flexibility === 'urgent' ? 'speed' : 'value',
        previousVendors: [],
  ...
  const calculateComplexity = () => {
    const heightScore = data.filesPhotos?.summary?.measurements?.stories > 5 ? 0.8 : 0.5;
    const serviceScore = services.length > 3 ? 0.7 : 0.4;
    const accessScore = data.scopeDetails?.accessRestrictions?.length > 0 ? 0.6 : 0.3;
  ...
  const loadHistoricalWinRates = async () => {
    ...
    return [
      ...
      { pricePoint: markedUpTotal * 1.0, winRate: 0.60, projects: 25 },
      { pricePoint: markedUpTotal * 1.1, winRate: 0.45, projects: 18 },
      { pricePoint: markedUpTotal * 1.2, winRate: 0.30, projects: 10 },
  ...
  const handlePriceOverride = (newPrice: number, reason: string) => {
    ...
    const discountPercentage = ((markedUpTotal - newPrice) / markedUpTotal) * 100;
  ...
  const calculateRiskFactors = () => {
    ...
    if (data.filesPhotos?.summary?.measurements?.stories > 5) {
      risks.push({
        ...
        description: 'Building height requires specialized equipment and safety measures',
        impact: 8,
        mitigation: 'Ensure proper equipment and certified operators',
        factors: ['Lift rental costs', 'Safety requirements', 'Slower production']
      });
    }
      ...
      <div className='bg-blue-50 rounded-lg p-6'>
        <div className='grid grid-cols-4 gap-4'>
          ...
          <div>
            ...
            <p className='text-xl font-bold'>${markedUpTotal.toLocaleString()}</p>
          </div>
          <div>
        ...
        <div className='fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50'>
          <div className='max-w-2xl w-full mx-4'>
            <DiscountApproval
              ...
              discountPercentage={((markedUpTotal - finalPrice) / markedUpTotal) * 100}
              reason={selectedStrategy?.adjustments?.[0]?.reason || 'Manual override'}
