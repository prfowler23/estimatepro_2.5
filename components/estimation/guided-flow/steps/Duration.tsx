...
import { Calendar, Cloud, AlertTriangle, Clock, TrendingUp, Download, Settings, CheckCircle, XCircle, Loader2 } from 'lucide-react';
...
interface WeatherAnalysis {
  ...
  forecast: {
    nextWeek: Array<{ date: string; risk: 'low' | 'medium' | 'high'; conditions: string }>;
...
const SERVICE_RATES = {
  'WC': { rate: 0.1, unit: 'sqft', weatherSensitive: true, name: 'Window Cleaning' },
  'GR': { rate: 0.15, unit: 'sqft', weatherSensitive: true, name: 'Glass Restoration' },
  'BWP': { rate: 0.08, unit: 'sqft', weatherSensitive: true, name: 'Building Wash (Pressure)' },
  'BWS': { rate: 0.12, unit: 'sqft', weatherSensitive: true, name: 'Building Wash (Soft)' },
  'HBW': { rate: 0.2, unit: 'sqft', weatherSensitive: true, name: 'High-Rise Building Wash' },
  'PWF': { rate: 0.05, unit: 'sqft', weatherSensitive: true, name: 'Pressure Wash (Flat)' },
  'HFS': { rate: 0.08, unit: 'sqft', weatherSensitive: false, name: 'Hard Floor Scrubbing' },
  'PC': { rate: 0.5, unit: 'space', weatherSensitive: true, name: 'Parking Cleaning' },
  'PWP': { rate: 0.06, unit: 'sqft', weatherSensitive: true, name: 'Parking Pressure Wash' },
  'IW': { rate: 0.15, unit: 'sqft', weatherSensitive: false, name: 'Interior Wall Cleaning' },
  'DC': { rate: 0.1, unit: 'sqft', weatherSensitive: false, name: 'Deck Cleaning' }
};
...
export function Duration({ data, onUpdate, onNext, onBack }: DurationProps) {
  ...
  const calculateDurations = async () => {
    ...
    try {
      ...
      const fallbackDurations = selectedServices.map((service: string) => ({
        ...
        dependencies: SERVICE_DEPENDENCIES[service as keyof typeof SERVICE_DEPENDENCIES] || []
      }));
  ...
  const handleManualOverride = (service: string, newDuration: number, reason: string) => {
    const updated = serviceDurations.map((sd: any) => 
      sd.service === service 
        ? { ...sd, finalDuration: newDuration, isOverridden: true, overrideReason: reason }
        : sd
    );
  ...
  const handleExportCalendar = () => {
    if (timeline) {
      ...
      const projectInfo = {
        name: data.initialContact?.extractedData?.customer?.company || 'Cleaning Project',
        location: location,
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">
          Intelligent duration calculation with weather analysis and timeline optimization
        </p>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Set start date and view calculated timeline</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              <label className="text-sm font-medium mb-2 block">Optimal Start Date</label>
            ...
            <div>
              <label className="text-sm font-medium mb-2 block">Weather Risk</label>
              <p className="text-sm px-3 py-2 bg-muted rounded">
                {weatherAnalysis?.riskScore ? (
                  weatherAnalysis.riskScore < 0.3 ? '✅ Low Risk' : 
                  weatherAnalysis.riskScore < 0.6 ? '⚠️ Medium Risk' : '❌ High Risk'
                ) : 'Calculating...'}
              </p>
            </div>
          </div>
