...
export function HistoricalComparison({
  ...
  const loadHistoricalData = async () => {
    ...
    const similar = data.recentProjects.filter((project) => {
      ...
      return (
        hasMatchingService &&
        (buildingType === "any" || project.buildingType === buildingType)
      );
    });
    ...
    <Card className="w-full">
      <CardHeader>
        ...
        <p className="text-sm text-muted-foreground">
          Based on {filteredProjects.length} similar projects • {buildingType}{" "}
          buildings • {services.join(", ")}
        </p>
      </CardHeader>
      ...
      <CardContent className="space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
            {Object.entries(variance).map(([category, value]) => (
              ...
              >
                ...
                <div
                  className={`flex items-center justify-center gap-1 ${getVarianceColor(value)}`}
                >
                ...
                <p className="text-xs text-gray-500 mt-1">
                  $
                  {category === "total"
                    ? currentCosts.total.toFixed(0)
                    : currentCosts[category as keyof CostSummary]?.toFixed?.(
                        0,
                      ) || "0"}
                </p>
              </div>
            ...
            <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg p-4">
              <div className="flex justify-between items-center mb-3">
                ...
                <div className="text-right">
                  ...
                  <p className="text-sm text-gray-600 mt-1">
                    {perSqftVariance > 0 ? "+" : ""}
                    {perSqftVariance.toFixed(1)}% vs avg
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-2">
                <div className="flex justify-between text-sm">
                  ...
                  <span>
                    ${historicalData.benchmarks.low.toFixed(2)} - $
                    {historicalData.benchmarks.premium.toFixed(2)}
                  </span>
                </div>
                <Progress
                  value={Math.min(
                    100,
                    (currentPerSqft / historicalData.benchmarks.premium) * 100,
                  )}
                  className="h-2"
                />
              </div>
        ...
        <div>
          <h4 className="font-medium mb-3">
            Recent Similar Projects ({filteredProjects.length})
          </h4>
          <div className="space-y-3 max-h-64 overflow-y-auto">
            {filteredProjects.map((project) => (
              ...
              >
                <div className="flex justify-between items-start mb-2">
                  ...
                  <div className="text-right">
                    <p className="text-lg font-bold">
                      ${project.costs.total.toLocaleString()}
                    </p>
                    <p className="text-sm text-gray-600">
                      ${project.perSqft.toFixed(2)}/sq ft
                    </p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-4 gap-2 text-xs text-gray-600 mt-3 pt-3 border-t">
                  <div>
                    Equipment: ${project.costs.equipment.toLocaleString()}
                  </div>
                  <div>
                    Materials: ${project.costs.materials.toLocaleString()}
                  </div>
            ...
            {filteredProjects.length === 0 && (
              <div className="text-center py-8 text-gray-500">
                ...
                <p className="text-sm">
                  Try adjusting the filters or timeframe
                </p>
              </div>
            )}
          </div>
        ...
        <Alert>
          ...
          <AlertDescription>
            ...
            year-over-year. Equipment rental (+
            {historicalData.priceInflation.equipment.toFixed(1)}%), materials (+
            {historicalData.priceInflation.materials.toFixed(1)}%), and labor (+
            {historicalData.priceInflation.labor.toFixed(1)}%) all show upward
            trends.
          </AlertDescription>
        </Alert>
        ...
        {Math.abs(variance.total) > 20 && (
          <Alert variant={variance.total > 20 ? "destructive" : "default"}>
            ...
            <AlertDescription>
              {variance.total > 20 ? (
                <>
                  <strong>Cost Alert:</strong> Your estimate is{" "}
                  {variance.total.toFixed(1)}% above historical averages.
                  Consider reviewing equipment rates, material quantities, or
                  labor allocations.
                </>
              ) : (
                <>
                  <strong>Competitive Pricing:</strong> Your estimate is{" "}
                  {Math.abs(variance.total).toFixed(1)}% below market averages.
