...
import { DrawingService, Shape, Measurement, Point } from '@/lib/canvas/drawing-service';
...
export function DrawingCanvas({ 
  ...
  const [drawingService, setDrawingService] = useState<DrawingService | null>(null);
  ...
  const updateShapeManager = useCallback((newShapes?: Shape[], newMeasurements?: Measurement[]) => {
    const shapes = newShapes || drawingService?.getShapes() || [];
    const measurements = newMeasurements || drawingService?.getMeasurements() || [];
  ...
  const selectShapeAt = useCallback((point: Point): boolean => {
    ...
    // Check shapes first
    for (const shape of shapes.reverse()) { // Check from top to bottom
    ...
    switch (currentTool) {
      ...
      case 'measure':
        if (currentPoints.length === 0) {
          ...
          try {
            const measurement = drawingService.drawMeasurementLine(currentPoints[0], point);
            updateShapeManager();
            setCurrentPoints([]);
          } catch (error) {
  ...
  }, [drawingService, currentTool, currentPoints, getMousePosition, selectShapeAt, updateShapeManager]);
  ...
  }, [drawingService, isDrawing, currentTool, currentPoints, getMousePosition, backgroundImage]);
  ...
  const handleMouseUp = useCallback((e: React.MouseEvent) => {
    ...
    if (width > 10 && height > 10) {
      try {
        const shape = drawingService.drawRectangle(currentPoints[0], endPoint);
        updateShapeManager();
      } catch (error) {
    ...
    setCurrentPoints([]);
    redrawCanvas();
  }, [drawingService, isDrawing, currentTool, currentPoints, getMousePosition, updateShapeManager, redrawCanvas]);
