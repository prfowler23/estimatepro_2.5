      ...
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6'>
        <Card>
          ...
          <CardContent>
            ...
            <div className={`flex items-center text-xs ${getTrendColor(data.overview.monthlyGrowth)}`}>
              {getTrendIcon(data.overview.monthlyGrowth)}
              <span className='ml-1'>
                {formatPercentage(Math.abs(data.overview.monthlyGrowth))} from last month
              </span>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className='text-2xl font-bold'>{data.overview.totalQuotes}</div>
        ...
        <Card>
          ...
          <CardContent>
            <div className='text-2xl font-bold'>
              {formatCurrency(data.overview.avgQuoteValue)}
            </div>
            <div className={`flex items-center text-xs ${getTrendColor(data.overview.yearOverYear)}`}>
              {getTrendIcon(data.overview.yearOverYear)}
              <span className='ml-1'>
                {formatPercentage(Math.abs(data.overview.yearOverYear))} vs last year
              </span>
            </div>
          </CardContent>
        </Card>
      ...
      <div className='grid grid-cols-1 lg:grid-cols-2 gap-6'>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Number of quotes created monthly</CardDescription>
          </CardHeader>
          <CardContent>
            <ResponsiveContainer width='100%' height={300}>
              <BarChart data={data.revenue.monthly}>
                ...
                <YAxis />
                <Tooltip />
                <Bar dataKey='quotes' fill='#10B981' />
              </BarChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
                ...
                <div key={service.serviceType} className='flex items-center justify-between'>
                  <div className='flex items-center space-x-3'>
                    ...
                    <div>
                      ...
                      <p className='text-sm text-gray-500'>
                        {service.totalQuotes} quotes • {formatPercentage(service.conversionRate)} conversion
                      </p>
                    </div>
                  </div>
                  <div className='text-right'>
                    ...
                    <div className='flex items-center'>
                      {service.trend === 'up' && <TrendingUp className='h-3 w-3 text-green-600 mr-1' />}
                      {service.trend === 'down' && <TrendingDown className='h-3 w-3 text-red-600 mr-1' />}
                      <span className={`text-xs ${
                        service.trend === 'up' ? 'text-green-600' : 
                        service.trend === 'down' ? 'text-red-600' : 'text-gray-500'
                      }`}>
      ...
      <div className='grid grid-cols-1 lg:grid-cols-2 gap-6'>
          ...
          <CardContent>
            <div className='grid grid-cols-2 gap-4 mb-6'>
              <div className='text-center p-4 bg-blue-50 rounded-lg'>
                <div className='text-2xl font-bold text-blue-700'>{data.customers.totalCustomers}</div>
                <div className='text-sm text-blue-600'>Total Customers</div>
              </div>
              <div className='text-center p-4 bg-green-50 rounded-lg'>
                <div className='text-2xl font-bold text-green-700'>{data.customers.newCustomers}</div>
              ...
              <div className='text-center p-4 bg-orange-50 rounded-lg'>
                <div className='text-2xl font-bold text-orange-700'>{data.customers.repeatCustomers}</div>
            ...
            <div className='space-y-3'>
              ...
              {data.customers.customersByType.map((type) => (
                <div key={type.type} className='flex items-center justify-between'>
                  ...
                  <div className='text-right'>
                    <div className='text-sm font-medium'>{type.count} customers</div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Quote progression through sales stages</CardDescription>
          </CardHeader>
          <CardContent>
            <ResponsiveContainer width='100%' height={300}>
              <FunnelChart>
        ...
        <CardContent>
          <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4'>
            {data.customers.topCustomers.slice(0, 6).map((customer, index) => (
              <div key={`${customer.name}-${customer.company}`} className='p-4 border rounded-lg'>
                ...
                <div className='space-y-1'>
                  ...
                  <div className='flex justify-between text-xs text-gray-500'>
                    ...
                    <span>{new Date(customer.lastQuote).toLocaleDateString()}</span>
                  </div>
                </div>
              </div>
            ))}
