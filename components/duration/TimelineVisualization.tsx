...
import { Calendar, AlertTriangle, CheckCircle, Clock, Users, Info, ArrowRight } from 'lucide-react';
...
export function TimelineVisualization({ 
    ...
    return (
      <Card>
        <CardContent className="p-8 text-center text-muted-foreground">
          ...
          <p className="text-sm">Add services and set a start date to view the project timeline</p>
        </CardContent>
      </Card>
    );
  }
  ...
  const startDate = timeline.reduce((min, entry) => 
    entry.startDate < min ? entry.startDate : min, 
    timeline[0].startDate
  );
  
  const endDate = timeline.reduce((max, entry) => 
    entry.endDate > max ? entry.endDate : max,
    timeline[0].endDate
  );
  
  const totalDays = Math.ceil((endDate.getTime() - startDate.getTime()) / (1000 * 60 * 60 * 24)) + 1;
  
  const getServiceColor = (service: string, weatherRisk: string, status?: string) => {
  ...
  const calculatePosition = (date: Date): number => {
    const daysSinceStart = Math.max(0, Math.ceil((date.getTime() - startDate.getTime()) / (1000 * 60 * 60 * 24)));
  ...
  const getSelectedServiceDetails = () => {
    ...
    const criticalPathServices = timeline.filter(t => t.isOnCriticalPath).map(t => t.service);
      ...
      <Card>
        <CardHeader>
          ...
          <div className="text-sm text-muted-foreground">
            {timeline.length} services • {totalDays} days • {startDate.toLocaleDateString()} to {endDate.toLocaleDateString()}
          </div>
        </CardHeader>
        ...
        <CardContent>
          {viewMode === 'timeline' ? (
            <div className="space-y-4">
                ...
                {weekMarkers.map((marker, index) => (
                  ...
                  >
                    <div className="text-center">
                      <div>{marker.date.toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</div>
              ...
              <div className="relative h-2 mb-2">
                {weekMarkers.filter(m => m.isWeekend).map((marker, index) => (
                  <div
                    key={index}
                  ...
                  return (
                    <div key={entry.service} className="relative h-12">
                      ...
                      <div className="ml-22 relative h-full">
                        ...
                        {weekMarkers.filter(m => m.isWeekend).map((marker, weekIndex) => (
                          <div
                            key={weekIndex}
                        ...
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <div
                              className={`absolute h-8 rounded-md cursor-pointer transition-all hover:shadow-md ${
                                getServiceColor(entry.service, entry.weatherRisk, entry.status)
                              } ${entry.isOnCriticalPath ? 'ring-2 ring-red-400 ring-offset-1' : ''} ${
                                selectedService === entry.service ? 'ring-2 ring-blue-400 ring-offset-1' : ''
                              }`}
                              style={{
                                left: `${left}%`,
                            ...
                            >
                              <div className="px-2 py-1 text-white text-xs flex items-center justify-between h-full">
                                <span className="font-medium">{entry.duration}d</span>
                          ...
                          <TooltipContent>
                            <div className="text-sm">
                              <div className="font-semibold">{entry.serviceName || entry.service}</div>
                              <div>Duration: {entry.duration} days</div>
                              <div>Dates: {entry.startDate.toLocaleDateString()} - {entry.endDate.toLocaleDateString()}</div>
                              <div>Weather Risk: <span className={`capitalize ${
                                entry.weatherRisk === 'high' ? 'text-red-500' : 
                                entry.weatherRisk === 'medium' ? 'text-yellow-500' : 'text-green-500'
                              }`}>{entry.weatherRisk}</span></div>
                              {entry.isOnCriticalPath && (
                                <div className="text-red-500 font-medium">Critical Path</div>
                              )}
                            </div>
                          </TooltipContent>
                        </Tooltip>
                        ...
                        {entry.dependencies.map(dep => {
                          const depEntry = timeline.find(t => t.service === dep);
                          ...
                          return (
                            <div key={dep} className="absolute top-1/2 transform -translate-y-1/2">
                              {/* Dependency line */}
                              <div
            ...
            <div className="space-y-4">
              {timeline.map((entry) => (
                <Card key={entry.service} className="cursor-pointer hover:shadow-md transition-shadow"
                      onClick={() => setSelectedService(entry.service)}>
                  <CardContent className="p-4">
                    <div className="flex justify-between items-start">
                      ...
                      <div className="flex flex-col items-end gap-2">
                        <Badge variant={
                          entry.weatherRisk === 'high' ? 'destructive' :
                          entry.weatherRisk === 'medium' ? 'secondary' : 'default'
                        }>
          ...
          {selectedService && showDetails && (
            <Card className="mt-6 border-blue-200">
              <CardHeader>
                <CardTitle className="text-lg">Service Details: {selectedService}</CardTitle>
              </CardHeader>
              <CardContent>
                {(() => {
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        ...
                        <div className="space-y-1 text-sm">
                          <div>Start: {details.startDate.toLocaleDateString()}</div>
                          <div>End: {details.endDate.toLocaleDateString()}</div>
                          <div>Duration: {details.duration} days</div>
                          {details.crewSize && <div>Crew Size: {details.crewSize} people</div>}
                        </div>
                      </div>
                      ...
                      <div>
                        ...
                        <div className="space-y-1 text-sm">
                          <div className="flex justify-between">
                            <span>Weather Risk:</span>
                            <Badge variant={
                              details.weatherRisk === 'high' ? 'destructive' :
                              details.weatherRisk === 'medium' ? 'secondary' : 'default'
                            } className="text-xs">
                              {details.weatherRisk}
                            </Badge>
                          </div>
                      ...
                      {details.overlaps.length > 0 && (
                        <div>
                          <h5 className="font-medium mb-2">Overlapping Services</h5>
                          <div className="space-y-1 text-sm">
                            {details.overlaps.map(overlap => (
                              <div key={overlap.service}>• {overlap.service}</div>
                            ))}
                          </div>
                        </div>
                      )}
                ...
                {onAdjust && (
                  <div className="mt-4 pt-4 border-t">
                    <Button 
                      ...
                      size="sm"
                      onClick={() => {
                        // TODO: Implement adjustment dialog
                        console.log('Adjust service:', selectedService);
                      }}
                    >
                      Adjust Schedule
                    </Button>
