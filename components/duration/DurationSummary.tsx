...
export function DurationSummary({
  ...
  const totalBaseDuration = serviceDurations.reduce(
    (sum, sd) => sum + sd.baseDuration,
    0,
  );
  const totalWeatherImpact = serviceDurations.reduce(
    (sum, sd) => sum + sd.weatherImpact,
    0,
  );
  const weatherSensitiveServices = serviceDurations.filter(
    (sd) => sd.weatherImpact > 0,
  ).length;
  ...
  const highConfidenceServices = serviceDurations.filter(
    (sd) => sd.confidence === "high",
  ).length;
  const lowConfidenceServices = serviceDurations.filter(
    (sd) => sd.confidence === "low",
  ).length;
          ...
          <div className="bg-red-50 border border-red-200 rounded-lg p-4">
            ...
            <div className="text-sm space-y-2">
              ...
              <p className="text-red-600 text-xs">
                Delays to these services will directly impact the project
                completion date.
              </p>
            </div>
          </div>
