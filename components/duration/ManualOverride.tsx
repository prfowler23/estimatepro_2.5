          ...
          <Card className="border border-blue-200 bg-blue-50/50">
            <CardContent className="p-4 space-y-4">
              <div className="space-y-3">
                      ...
                      {availableServices.map(sd => (
                        <SelectItem key={sd.service} value={sd.service}>
                          <div className="flex justify-between items-center w-full">
                            <span>{SERVICE_NAMES[sd.service] || sd.serviceName || sd.service}</span>
                ...
                <div>
                  <label className="text-sm font-medium mb-2 block">New Duration (days)</label>
                  <Input
                    type="number"
                    step="0.5"
                    min="0.5"
                ...
                {selectedService && newDuration && (
                  <Card className="bg-blue-50 border-blue-200">
                    <CardContent className="p-3">
                      <h5 className="font-medium text-sm mb-2">Impact Preview</h5>
                      <div className="text-xs space-y-1">
                        <div className="flex justify-between">
                          <span>Original Duration:</span>
                          <span>{getSelectedServiceData()?.baseDuration || 0} days (base)</span>
                        </div>
                        <div className="flex justify-between">
                          <span>Weather Impact:</span>
                          <span>+{getSelectedServiceData()?.weatherImpact || 0} days</span>
                        </div>
                        <div className="flex justify-between">
                          <span>Current Total:</span>
                          <span>{getSelectedServiceData()?.finalDuration || 0} days</span>
                        </div>
              ...
              <div className="flex gap-2">
                <Button onClick={handleSubmit} disabled={!selectedService || !newDuration || !reason}>
                  Apply Override
                </Button>
          ...
          <div>
            ...
            <div className="space-y-3">
              {overriddenServices.map(sd => (
                <Card key={sd.service} className="border-l-4 border-l-orange-400">
                        ...
                        <div className="grid grid-cols-2 gap-4 text-sm mb-3">
                          <div>
                            ...
                            <span className="ml-2 font-medium">
                              {sd.originalDuration || sd.baseDuration + sd.weatherImpact} days
                            </span>
                          </div>
                          <div>
                            <span className="text-muted-foreground">Override:</span>
                            <span className="ml-2 font-medium">{sd.finalDuration} days</span>
                          </div>
                        </div>
            ...
            <CardContent className="p-3">
              <div className="flex items-center gap-2 text-sm">
                ...
                <span className="font-medium text-orange-800">
                  {overriddenServices.length} service{overriddenServices.length !== 1 ? 's' : ''} 
                  {overriddenServices.length === 1 ? ' has' : ' have'} manual duration adjustments
                </span>
              </div>
              <p className="text-xs text-orange-700 mt-1">
                These overrides will be preserved when recalculating the project timeline
              </p>
            </CardContent>
          </Card>
        )}
