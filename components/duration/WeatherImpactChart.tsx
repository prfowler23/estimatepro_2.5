...
import { CheckCircle, AlertTriangle, Cloud, Thermometer, Wind, Droplets } from 'lucide-react';
...
const SERVICE_WEATHER_SENSITIVITY: Record<string, ServiceWeatherSensitivity> = {
  ...
  'BWS': { rain: 'critical', temperature: 'high', wind: 'medium', snow: 'critical' },
  'HBW': { rain: 'high', temperature: 'medium', wind: 'critical', snow: 'critical' },
...
export function WeatherImpactChart({ 
  ...
  const getServiceWeatherSensitivity = (service: string): ServiceWeatherSensitivity => {
    return SERVICE_WEATHER_SENSITIVITY[service] || {
      rain: 'medium',
      temperature: 'medium',
      wind: 'medium',
      snow: 'medium'
      ...
      <CardContent>
        <Tabs defaultValue="workability" className="w-full">
            ...
            <div>
              ...
              <ResponsiveContainer width="100%" height={300}>
                <BarChart data={workabilityData}>
                  ...
                  <Legend />
                  <Bar dataKey="workableDays" fill="#10b981" name="Workable Days" />
                  <Bar dataKey="rainDays" fill="#3b82f6" name="Rain Days" />
                  <Bar dataKey="snowDays" fill="#6366f1" name="Snow Days" />
                  <Bar dataKey="extremeTempDays" fill="#f59e0b" name="Extreme Temp Days" />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </TabsContent>
          ...
          <TabsContent value="precipitation" className="space-y-4">
            <div>
              <h4 className="font-medium mb-3">Precipitation and Wind Patterns</h4>
              <ResponsiveContainer width="100%" height={300}>
                <LineChart data={precipitationData}>
                  ...
                  <Tooltip content={<CustomTooltip />} />
                  <Legend />
                  <Bar yAxisId="left" dataKey="precipitation" fill="#06b6d4" name="Precipitation (in)" />
                  <Line 
                    yAxisId="right" 
                    type="monotone" 
                    dataKey="windSpeed" 
                  ...
                  return (
                    <Card key={service} className="p-4">
                      <div className="flex justify-between items-start mb-3">
                        <div>
                          <h5 className="font-medium">{SERVICE_NAMES[service] || service}</h5>
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div className="flex items-center justify-between">
                          ...
                          <Badge variant={getSensitivityBadgeVariant(sensitivity.rain)} className="text-xs">
                            {sensitivity.rain}
                          </Badge>
                        </div>
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <Badge variant={getSensitivityBadgeVariant(sensitivity.temperature)} className="text-xs">
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <Badge variant={getSensitivityBadgeVariant(sensitivity.wind)} className="text-xs">
                            {sensitivity.wind}
                          </Badge>
                        </div>
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <Badge variant={getSensitivityBadgeVariant(sensitivity.snow)} className="text-xs">
                            {sensitivity.snow}
                          </Badge>
                        </div>
                      </div>
        ...
        <Card className="mt-6 bg-gray-50">
          <CardHeader>
            <CardTitle className="text-sm">Weather Sensitivity Levels</CardTitle>
          </CardHeader>
          <CardContent>
