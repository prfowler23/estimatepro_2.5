...
export function MeasurementTable({ 
  ...
  const addEntry = () => {
    const newEntry: MeasurementEntry = {
      ...
      height: 0,
      quantity: 1,
      unit: calculation === 'area' ? 'sqft' : calculation === 'linear' ? 'lf' : 'ea',
      total: 0
    };
  ...
  const updateEntry = (id: string, field: string, value: any) => {
    const updated = entries.map(entry => {
      if (entry.id === id) {
        ...
        if (calculation === 'area') {
          updatedEntry.total = updatedEntry.width * updatedEntry.height * updatedEntry.quantity;
        } else if (calculation === 'linear') {
          updatedEntry.total = (updatedEntry.width || updatedEntry.length || 0) * updatedEntry.quantity;
        } else {
        ...
        <table className='w-full'>
          <thead className='bg-gray-50 border-b'>
            <tr>
              ...
              <th className='px-4 py-2 text-center text-xs font-medium text-gray-500'>Width (ft)</th>
              <th className='px-4 py-2 text-center text-xs font-medium text-gray-500'>
                {calculation === 'area' ? 'Height (ft)' : 'Length (ft)'}
              </th>
              <th className='px-4 py-2 text-center text-xs font-medium text-gray-500'>Qty</th>
              <th className='px-4 py-2 text-center text-xs font-medium text-gray-500'>
                Total ({calculation === 'area' ? 'sq ft' : calculation === 'linear' ? 'lf' : 'ea'})
              </th>
              ...
              <tr key={entry.id} className='hover:bg-gray-50'>
                ...
                <td className='px-4 py-2'>
                  <input
                    type='number'
                    value={entry.width}
                    onChange={(e) => updateEntry(entry.id, 'width', parseFloat(e.target.value) || 0)}
          ...
          <tfoot className='bg-gray-50 border-t'>
            <tr>
              ...
              <td className='px-4 py-2 text-center font-bold'>{totalArea.toFixed(2)}</td>
              <td></td>
            </tr>
          </tfoot>
        </table>
