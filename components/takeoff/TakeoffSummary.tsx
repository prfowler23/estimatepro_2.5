...
export function TakeoffSummary({ measurements, services }: TakeoffSummaryProps) {
  ...
  const grouped = measurements.reduce((acc, m) => {
    ...
    return acc;
  }, {} as Record<string, { entries: MeasurementEntry[], total: number, unit: string }>);
  ...
  const serviceTotals = services.map(service => {
    ...
    switch (service) {
      case 'WC':
      case 'GR':
        total = (grouped['glass_windows']?.total || 0) + 
      ...
      case 'HBW':
        ...
        unit = 'sqft';
        categories = Object.keys(grouped).filter(key => key.startsWith('facade_'));
        break;
      case 'PWF':
      case 'HFS':
    ...
    return { service, total, unit, categories, name: serviceNames[service] || service };
  });
    ...
    <div className='bg-blue-50 p-6 rounded-lg'>
      ...
      <div className='grid grid-cols-3 gap-4 mb-6'>
        <div className='bg-white p-3 rounded text-center'>
          <p className='text-2xl font-bold text-blue-600'>{totalMeasurements}</p>
          <p className='text-sm text-gray-600'>Total Entries</p>
        </div>
        <div className='bg-white p-3 rounded text-center'>
          <p className='text-2xl font-bold text-green-600'>{totalArea.toFixed(0)}</p>
          <p className='text-sm text-gray-600'>Total Area (sqft)</p>
        </div>
        <div className='bg-white p-3 rounded text-center'>
          <p className='text-2xl font-bold text-orange-600'>{Object.keys(grouped).length}</p>
        ...
        <div className='mb-6'>
          ...
          <div className='grid grid-cols-2 gap-3'>
            {Object.entries(grouped).map(([category, data]) => (
              <div key={category} className='bg-white p-3 rounded border-l-4 border-blue-200'>
                <p className='text-sm text-gray-600 capitalize'>
                  {category.replace(/_/g, ' ').replace('facade ', '').replace('glass ', '').replace('parking ', '')}
                </p>
      ...
      <div className='mb-4'>
        ...
        <div className='space-y-2'>
          {serviceTotals.map(({ service, total, unit, name }) => (
            <div key={service} className={`flex justify-between items-center p-2 rounded ${
              ...
              <div className='text-right'>
                <span className={`font-medium ${total === 0 ? 'text-red-600' : 'text-gray-800'}`}>
      ...
      {missingMeasurements.length === 0 && services.length > 0 && (
        <div className='bg-green-50 border border-green-200 p-3 rounded'>
          <p className='text-sm font-medium text-green-800 mb-1'>✅ All Requirements Met</p>
          <p className='text-xs text-green-600'>
            All selected services have the required measurements for accurate estimation.
          </p>
        </div>
      )}
      ...
      {totalMeasurements === 0 && (
        <div className='bg-yellow-50 border border-yellow-200 p-4 rounded text-center'>
          <p className='text-sm font-medium text-yellow-800 mb-1'>No Measurements Added</p>
          <p className='text-xs text-yellow-600'>
            Start by adding measurements for your selected services to see the takeoff summary.
          </p>
        </div>
      )}
    </div>
